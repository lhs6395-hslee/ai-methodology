...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-27 | FR-005 보강 — 소유하는 spec이 없는 `entitySchemaExemptEntities` 항목(dangling)을 매 실행 경고로 표면화. `entityRegistry`의 dangling 경고(registryWarns)와 대칭 — 이전엔 면제 등록부만 이 보호가 없었다. Node·Python 패리티 | 감사 이슈 #21 사소 항목: entityRegistry는 미소유 등록 항목을 매 실행 지목하는데(check-ownership.mjs registryWarns) entitySchemaExemptEntities는 *사용 중인* 면제만 부채로 표면화하고(FR-005 기존) 미사용 면제는 조용히 방치돼 두 등록부가 비대칭이었다 [검증: tooling/__tests__/schema-backing.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs] |
