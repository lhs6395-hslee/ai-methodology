...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-17 | `tooling/harness/commit-msg`(소비 프로젝트 배포 템플릿)에 `check-duplicate-source-drift.mjs`·`check-invariant-guard.mjs`(SPEC-059, R26) 호출 단계 추가(`check-gate-escalation.mjs` 뒤) | 신규 게이트 무결성 감사 축이 실제로 소비 프로젝트의 commit-msg 훅에 배선돼야 `/sdd-update`로 받은 프로젝트에서 동작한다 — 라이브러리·게이트 파일만 배포하고 훅 템플릿을 안 고치면 그 프로젝트에서는 코드가 있어도 아무도 부르지 않는다 |
