...
- **FR-009** (event): WHEN **check-orphan-surfaces.mjs** (S) runs and `surfaceGlobs` is non-empty, THE SYSTEM SHALL report any surface file matched by `surfaceGlobs` that is not declared in any spec's `Ownership` block as an orphan advisory, exiting zero in advisory mode and non-zero under `--strict`; WHERE `surfaceGlobs` is empty, THE SYSTEM SHALL exit zero as a no-op. A declared surface key is matched against a candidate path by compiling it as a glob when it contains glob metacharacters, and otherwise only at a path boundary — exact equality or a `/`-delimited prefix/suffix — never by mid-string substring containment.
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | `check-pre-edit`의 소유 스펙 해석을 **FR 인덱스 우선**으로 전환(SPEC-062 FR-008) — 인덱스가 최신이면 `sdd/FR_INDEX.json` 한 파일만 읽고, 낡거나 없으면 종전대로 스펙 전수를 읽는다(출력·판정 불변, Node↔Python 바이트 동일 유지) | 훅은 편집마다 도는 자리라 비용이 곧 사용성이다 — 종전 판은 매 편집에 스펙 전수(킷 실측 63개·1.1MB)를 읽어 스펙이 늘면 선형으로 느려졌다(오너 제보: "뭐 하나 바꾸는데 너무 오래 걸린다"). 판정 규칙은 SPEC-062 소관이고 이 스펙은 훅 소유자로서 재료 조달 경로만 바꿨다 — 폴백을 남긴 이유는 인덱스가 캐시이고 정본이 아니기 때문이다 [검증: tooling/__tests__/fr-index.test.mjs, tooling/__tests__/fr-locator.test.mjs] |
| 2026-08-27 | FR-009 개정 — `check-orphan-surfaces`의 표면↔선언 대조를 양방향 부분문자열 포함에서 **경로 경계 일치**(정확 일치 또는 `/` 접두·접미)로 교체 + 글롭 메타문자가 있는 선언은 `compileGlob`으로 실제 컴파일해 대조(Node·Python 동시) | 감사 이슈 #21 M-1 — `norm(rel).includes(d) \|\| d.includes(norm(rel))`가 중간에서 끊기는 부분문자열도 그대로 인정해, 선언에 `src` 같은 짧은 토큰만 있어도 그 문자열을 담은 모든 표면이 조용히 "선언됨"으로 오판정됐다(실측: 소비 프로젝트 PM 고아 표면 오탐 78% — 18/23). Files·specSyncExemptGlobs가 이미 쓰는 `compileGlob`을 재사용해 글롭 선언은 정식 문법으로, 리터럴 선언은 경계 인식 일치로 갈랐다 [검증: tooling/__tests__/check-orphan-surfaces.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs] |
| 2026-08-27 | `collectDeleted`/`collect_deleted`가 **리네임(R) 원본 경로도** `deleted` 집합에 담도록 확장(FR-010) — Node·Python 동시 | 이슈 #27(외부 제보) — FR-010이 삭제(D)에 대해 해소한 ⓐⓑ 교착이 리네임(R)에는 빠져 있었다. `collectRenames`가 새 경로(m[2])만 담고 사라진 원본(m[1])은 아무도 기록하지 않아, 소유 파일을 `git mv`로 옮기면 스펙이 가리키는 옛 경로가 "잘못 적힌 경로"로 오판돼 **어떤 커밋 순서로도 정답이 없었다**(실측 규모: 소비 프로젝트에서 파일 203건 리네임에 스펙 14개가 차단, `--no-verify` 외 길이 없었음). `renamed`(SPEC-019 semantic drift 트리거)는 그대로 둔다 — 리네임 자체는 여전히 의미 변경을 요구한다, 없앤 것은 잘못된 진단 하나뿐 [검증: tooling/__tests__/check-spec-sync.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs] |
...
| 2026-08-10 | 편집 시점 spec-first의 **Python 판정 경로** 확보 — 훅 쉘이 런타임을 골라 부른다(node 게이트 → python3 `preedit`) | 차단 강도를 준 뒤에도 Node만 판정할 수 있으면 Python 런타임 프로젝트는 `hard`를 켜고 **보호 0**이다 — 이 스펙이 신설한 강제가 그 프로젝트에서는 선언뿐이 된다 |
| 2026-08-11 | `tooling/harness/commit-msg`(소비 프로젝트 설치 원본)에 `check-fr-placement.mjs`·`check-gate-escalation.mjs`를 `check-spec-sync.mjs` **뒤에** 순차 체인(각 단계 실패 시 즉시 그 exit로 종료) | SPEC-056·057 동반. 스펙이 애초에 동반됐는지가 그 안의 FR 배치보다 우선이다 — 순서를 바꾸면 스펙 없는 커밋에도 배치 오류를 먼저 말해 사람이 엉뚱한 곳을 고친다 |
