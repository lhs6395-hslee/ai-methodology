...
- **설치·업데이트가 인덱스를 만든다(SPEC-004 FR-002).** "만들라"는 안내만 두면 아무도 만들지 않고, 그러면 조회가 계속 폴백(스펙 전수 읽기)으로 돌면서 **느려진 사실을 아무도 모른다** — 폴백은 출력으로 알리지만 그 출력을 보려면 이미 조회를 해야 한다. 그래서 `sdd-init`가 끝나는 자리에서 스펙이 있으면 한 번 생성한다(`/sdd-update`가 `sdd-init` 재실행이므로 업데이트마다 갱신된다). 생성 실패는 설치를 막지 않는다 — 캐시이고 정본은 스펙이다.
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | 설치·업데이트 프라이밍을 Edge Case로 명시 — 인덱스 생성은 `sdd-init`(SPEC-004 FR-002)이 끝나는 자리에서 자동으로 일어난다 | 조회 축을 만들어도 인덱스가 없으면 폴백만 남고, 폴백은 조회를 해봐야 보인다 — 발동을 사람 기억에 맡기지 않는다(이 킷이 반복해 고발하는 결함 클래스). 소비 프로젝트(AI PM) 도달 경로가 `/sdd-update`이므로 그 경로에 걸었다 [검증: tooling/__tests__/init-gates.test.mjs] |
