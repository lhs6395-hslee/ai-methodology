...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-27 | `unanchoredOwnedKeyFindings`가 `*` 포함 글롭 키를 앵커 요구에서 면제(B-1) + `buildKeySet`/`buildKeyKindMap`이 cfg를 받으면 정본 표기(`ownership-keys.normalizeKey`)도 원시 표기와 함께(상집합) 등록(B-3), Node·Python 패리티 | 감사 이슈 #21. B-1: `**GET /api/*/users**`처럼 `*`를 포함한 bold 앵커는 `\*\*([^*]+?)\*\*` 정규식이 `*` 내부 경계에서 항상 끊겨 구조적으로 추출 불가능한데, 이 카테고리에도 앵커를 요구해 hard에서 영구 차단됐다(요구할 수 없는 것을 요구함). B-3: 이 파일의 키 저장이 트림+소문자만 했고 `ownership-keys.normalizeKey`(METHOD 대문자·param 표준형)를 쓰지 않아, Ownership이 원시 표기(`:id`)로 선언돼도 DEDUP.md 권장 정본형(`{id}`)으로 앵커한 저자가 "키 아님"으로 오탐됐다 — 원시 표기를 지우지 않고 정본 표기를 더해 상집합으로 만들어 회귀 없이 해소 [검증: tooling/__tests__/key-anchor.test.mjs] |
...
| 2026-08-09 | `buildKeyKindMap`의 역할 이름 폴백을 `resolveCategoryRoles`(SPEC-001)로 위임 | 같은 이름 정규식(`entit`/`surface`/`capabilit`)이 두 사이트에 복제돼 있었고(R13 실측), 갈라지면 같은 카테고리를 두 사이트가 다르게 읽는다. 판정 내용 불변 — 폴백 정본을 하나로 |
