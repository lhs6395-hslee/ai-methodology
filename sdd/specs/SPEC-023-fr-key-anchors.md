...
- **굵게 ⟺ 사유 있는 키(양방향 규율, owner 요구 "굵은 글씨가 왜 굵은지 알 수 있어야").** FR 선언 라인에서: (a) 굵은 토큰은 반드시 선언 키여야 하고(아니면 위반 — FR-003), (b) 굵은 키는 반드시 그 카테고리 마커를 달아야 하며(FR-005), (c) 선언 키를 **백틱(리터럴)에 두면 위반**(FR-006 — 키는 앵커여야). 결과: 남는 굵은 것은 전부 "마커 달린 선언 키"(= 사유가 있음), 선언 키는 백틱이 아니라 굵게+마커, 비키(필드·enum·경로)는 백틱/평문. 단, 순수 산문 속 개념 언급(키 문자열이 그대로 안 나오는)까지 굵게 강제하는 것은 기계 판정 불가 — 저술 지침으로 남긴다(백틱·굵은 것만 판정).
