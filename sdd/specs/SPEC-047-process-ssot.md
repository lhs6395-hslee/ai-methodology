...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-17 | `unenforcedInvariantFindings`가 boolean 판정기 대신 TRI(SPEC-054)를 받아 `{violations, unchecked}`를 반환하도록 개정 — 실재 확인 못 함이 "없다"(거짓 위반)로 붕괴하지 않는다. `check-process-ssot.mjs`는 주입 함수를 `triGuard`로 감싼다. Python 미러·유닛 1건 동반 | 킷 자기적용 계약 테스트(check-outcome.test.mjs, SPEC-054 FR-004)가 "존재 판정기(`fileExists`)를 받는 코어는 확인 못 함 통로를 가져야 한다"를 이 파일에서 위반으로 지목했다 — FR-005 최초 구현은 boolean만 받아 읽기 실패를 "부재"로 붕괴시켰다 |
