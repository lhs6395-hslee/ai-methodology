...
| 2026-08-14 | `방법론.html` R25 절에 "이 축은 같은 프로그램이 두 모드로 돈다"(`--hook` 유무로 갈리는 훅/스윕) 문단 추가 — 훅 모드(행동 자체 판정)와 스윕 모드(`riskyActionPatterns` 선언 품질 판정)의 대상이 다름을 명시하고, 배선 실재는 R19가 별도로 본다는 3층 구분을 덧붙였다 | 오너 실측 지적: R25가 hook/record/sweep 3모드로 도는데(HARNESS.md R25 행은 형제 규칙 R21처럼 이미 훅/스윕을 나눠 서술했음) 이 문서만 훅 모드만 설명하고 스윕 모드(선언 완전성 자기점검)를 빠뜨리고 있었다 — SPEC-058 자체(FR-005, Dedup-Review)엔 이미 정확히 있었지만 상시 로드 문서엔 반영이 늦었다. 규칙 신설이 아니라 기존 R25 절의 누락 보완이라 규칙표·인용 수치는 불변 |
