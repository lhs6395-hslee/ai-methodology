...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | `docs/방법론.html`의 인용 수치 갱신 — 게이트 30→32 · 스펙 61→63 | 이 스펙의 hard 게이트가 직접 잡았다[실측]: SPEC-061·062와 그 게이트 2종이 들어오며 문서가 인용한 숫자가 낡았고 `check-intro-doc`이 커밋을 막았다 — "숫자는 가장 먼저 낡고 가장 늦게 들킨다"는 이 축의 전제가 자기 문서에서 발화한 사례 [검증: tooling/__tests__/intro-doc.test.mjs] |
| 2026-08-16 | R25 절에 훅/스윕 모드 구분 단락 + MCP 도구 커버리지 확장(`tool` 필드) 단락 추가 | 실측: R25 서술이 훅 모드(행동 자체)만 설명하고 스윕 모드(선언 품질 자체 판정)를 빠뜨렸다 — 형제 게이트 R21의 서술과 대비돼 드러났다. 킷 자신이 `mcp__github__merge_pull_request`로 PR을 머지하는 동안 이 축이 완전히 우회된 실측도 함께 적었다(SPEC-058 동반) [검증: tooling/__tests__/intro-doc.test.mjs] |
| 2026-08-10 | 소개 문서에 R21 절 추가("읽기가 재량이면 급할 때 가장 먼저 생략된다") + 인용 수치 3종 갱신(rules 19→20·gates 25→26·specs 53→54) | R15 동반 갱신. 이 절의 본체는 **명세 행번호 대조 블록**이다 — 61행·48행·273행에 답이 있었고 실제 원인은 다른 층이었다는 네 줄을 나란히 보지 않으면 "명세에 있었는데 왜 못 봤나"가 전달되지 않는다. 두 강도 표(surface/deny)와 "명세 읽기가 모든 규칙을 이긴다"도 함께 적었다 — 규칙의 신뢰는 그 규칙이 무엇을 **안 막는지** 아는 데서 온다 [검증: tooling/__tests__/intro-doc.test.mjs] |
| 2026-08-10 | 소개 문서에 R20 절 추가("감시와 감사는 다른 층이다") + 인용 수치 3종 갱신(rules 18→19·gates 24→25·specs 52→53) | R15가 빠진 규칙 설명과 세 숫자를 각각 지목해 막았다. 이 절의 본체는 **층 표**(감시 게이트/감시 에이전트/감사)와 **감사의 절반 표**(결정적=차단 / 의미=열거·판정 분업)다 — 오너가 셋으로 정리한 구조를 그 두 표 없이 산문으로만 적으면 "왜 감사를 감시에 겸하면 안 되는가"(순환)가 전달되지 않는다. 오탐 억제 3단계 조정 이야기도 남겼다: 규칙의 신뢰는 그 규칙이 무엇을 **안 잡는지** 아는 데서 온다 [검증: tooling/__tests__/intro-doc.test.mjs] |
| 2026-08-10 | 소개 문서에 면제 래칫 절 추가("면제는 걷어내는 사람이 없다") | R15 동반 갱신. 이 절의 본체는 **두 종류 표**다 — boundary는 "왜 영구인가", debt는 기한까지. 그 구분 없이 "면제엔 4필드"만 적으면 구조적 경계에 거짓 날짜가 생기고, 왜 그게 날짜 없음보다 나쁜지는 표 하나로 전달되지 않아 산문으로 적었다. 제보자의 자기관찰("반사적으로 면제를 떠올렸다")과 오너의 차단 문장도 함께 남겼다 — 규칙의 근거가 사람의 실제 반응일 때 그 이야기가 규칙보다 오래 기억된다 [검증: tooling/__tests__/intro-doc.test.mjs] |
...
| 2026-08-10 | `방법론.html`에 3분류 반환 계약 절 신설(양방향 실측 표 + 셋뿐인 종류 표 + 왜 코어에 있어야 하는가) + 인용 수치 `specs` 54→55 | 게이트가 낡은 숫자를 잡았다 — **숫자는 가장 먼저 낡고 가장 늦게 들킨다.** 새 축을 세우면 설명도 같은 커밋에서 따라와야 한다 |
| 2026-08-10 | 게이트 종수 인용의 스윕 규칙표 파싱을 `sweepGateFiles` 코어로 이전(중복 구현 제거) | 같은 정본을 두 곳에서 파싱하면 한쪽이 뒤처진다 — R13이 보는 결함이 정확히 그것이고, 이 숫자와 R17의 "강제 층 커버리지"는 **같은 정본**을 읽어야 한다 |
| 2026-08-10 | `방법론.html`에 양판 대응 기계화 절 신설 — 네 곳에서 같은 날 나온 같은 결함(CI 손목록 10종·설치기 목록 1종·픽스처 목록 8곳·양판 대응 2종)을 한 표로 묶었다 | 넷 다 "추가하면 여기에도 추가한다"는 규범이 붙어 있었다. **규범으로만 적힌 의무는 반드시 새어나간다** — 새로 배우는 사람이 이 문서로 방법론을 만나므로 그 교훈이 문서에 있어야 한다 |
| 2026-08-10 | `방법론.html`에 R22 절 신설(신호 3종 표 + 증거 등급과의 직교 + 값싼 해소를 안내하지 않는 이유) + 인용 수치 3종 갱신(규칙 21·게이트 27·스펙 56) | 새 규칙은 설명과 같은 커밋에서 착지한다. 게이트가 낡은 숫자 3건을 즉시 잡았다 — 숫자는 가장 먼저 낡고 가장 늦게 들킨다 |
| 2026-08-11 | `방법론.html`에 R23(FR 배치)·R24(게이트 실패 에스컬레이션) 절 신설 + 인용 수치 3종 갱신(규칙 23·게이트 29·스펙 58) | SPEC-056·057 동반. 게이트가 낡은 숫자 2건(게이트·스펙)을 즉시 잡았다 — 규칙 수는 두 규칙 신설을 이 문서가 직접 반영해야 검산을 통과한다 |
| 2026-08-13 | `방법론.html` §spec-first에 "spec-first는 진입 순서까지 뭉개지 않는다" 절 신설 — 신규 기능(spec부터)·버그 수정(재현부터) 두 갈래 카드 + 공통 규칙(같은 커밋이면 됨, 디스크 작성 순서 무관) | SPEC-004 동반(`agent-context.md`·`speckit-fix.SKILL.md` 개정과 같은 결정). 새로 배우는 사람이 이 문서로 방법론을 만나는데, 기존 서술은 spec-first를 커밋 계약으로는 정확히 설명하면서도 "손으로 무엇부터 쓰는가"는 명시하지 않아 버그 수정에도 스펙 문구부터 쓰는 오독을 유발했다 — 실제로 재현 전 추측성 스펙 문구를 썼다가 원인이 다른 곳이었던 사례가 있었다. 규칙 신설이 아니라 상시 로드 문서에서 이미 고친 구분을 이 문서에도 반영한 것이라 규칙표·인용 수치는 불변 |
| 2026-08-22 | `방법론.html`에 "계획된 인프라 변경이 이 changeset 범위를 벗어나는가(R8 확장)" 절 신설(R21·R8·R25와의 층 비교표에 행 추가 + `deployScopeCommand` 계약 + `SDD_ALLOW_DRIFT` 동의 변수 설명) | SPEC-035 FR-009 동반. 새 규칙 번호가 아니라 기존 R8의 확장이라 규칙표 행수·인용 수치는 늘지 않았다. 실측 제보(SNS 제거 작업 중 무관 리소스 6건 drift)를 애플리케이션 층 `check-spec-sync`와 나란히 두어, 새로 배우는 사람이 IaC 채택 시 이 축도 동기화 대상임을 처음부터 알게 한다 |
| 2026-08-14 | `방법론.html`에 R25(위험 행동 승인) 절 신설(R21·R8과의 층 비교표 + 해시 결속 이유 + 게이트가 서브에이전트를 스스로 안 부르는 이유 + 사례① 명시적 제외) + 인용 수치 3종 갱신(규칙 24·게이트 30·스펙 59) | SPEC-058 동반. 게이트가 낡은 숫자 3건을 즉시 잡았다 — 새 규칙은 설명과 같은 커밋에서 착지한다 |
| 2026-08-10 | `방법론.html`에 "추측의 성공은 침묵할 수 없다 / 금지할 수단이 없으면 금지가 아니다" 절 신설 | 오너가 못박은 두 규범(추측 금지·명세를 읽지 않고 멋대로 하기 금지)을 킷이 지키고 있는지 **측정**한 결과와 그 구멍을 문서에 남겼다 — 새로 배우는 사람은 이 문서로 방법론을 만나므로, "폴백은 실패할 때만 사유를 남겼다"·"종착지가 도달 불가한 사다리는 사다리가 아니다"·"열거기가 잘못된 집합을 보면 전수성이 아니다" 세 교훈이 여기 있어야 한다. 규칙 신설이 아니라 기존 축의 구멍 메우기라 규칙표 행은 늘지 않았다 |
| 2026-08-10 | `방법론.html`에 "재검토 조건은 방아쇠를 빠뜨릴 수 있다" 절 신설 | 이 방법론은 기각한 대안마다 재검토 조건을 적게 하는데, 그 장치가 실제로 시험받은 사례를 남겼다 — 조건이 방아쇠를 하나만 적었고 위험은 적히지 않은 경로로 왔다. **해소 방식이 요점이다**: 결정을 몰래 뒤집지 않고 조건을 갱신하고 그 처방을 이행했으며, 방아쇠가 터지지 않은 이웃 결정은 그대로 뒀다(근거 없는 확대 금지). 새로 배우는 사람이 재검토 조건을 "적어두면 끝"으로 오해하지 않게 하려면 이 사례가 문서에 있어야 한다 |
