...
- FR 선언 수집은 `cfg.__frDeclRe` 단일 문법(SPEC-001 FR-009)을 그대로 소비한다 — 게이트가 자체 정규식을 두면 사이트별 `requirementIdPrefixes`(도메인 접두어 INFRA-NNN 등)가 조용히 빠진다. 따라서 본문 산문에 볼드로 적힌 요구 ID는 이미 선언으로 집계되며(팬텀 FR), 그 결과 중복 hard로도 드러난다 — 산문에 리터럴 볼드 요구 ID를 쓰지 않는 SPEC-002 규칙의 기계 보강.
...
- **FR-005** (unwanted): IF one spec declares the same requirement id more than once, THEN the **spec-id-numbering** (E) judgment in **numbering-lib.mjs** (S) SHALL exit non-zero naming that spec-qualified requirement id and pointing at the retag path, evaluating each spec independently so identical numbers in different specs never collide (the spec id is already the namespace).
