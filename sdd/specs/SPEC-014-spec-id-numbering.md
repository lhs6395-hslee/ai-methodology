...
| 2026-08-02 | `frNumberingIssues`에 `declaredNums` 인자 추가 — 결번 advisory 문구를 폐기 흔적/선언됐으나 본문 없음 두 갈래로 분기(판정 소스는 SPEC-037 코어). Node·Python 동시 | 실측 제보: "FR 폐기 잔분일 수 있음" 한 문장이 **의미가 정반대인 두 상태**를 덮었다 — 정당한 흔적과 결함이 같은 말로 나오니 매 실행 로그에 섞여 흘러갔고, 그 사이 3개 FR이 몇 달간 SSOT 밖에 있었다. 인자 기본값이 빈 집합이라 미배선 호출부의 출력은 바이트 불변 [검증: tooling/__tests__/changelog-fr.test.mjs] |
