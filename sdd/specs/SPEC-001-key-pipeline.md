...
- **FR-005** (unwanted): WHERE `surfaceFormat` is `http` (default), IF a Surfaces key does not match `<METHOD> <path>` or a scheme configured in `surfacePathParam`'s sibling `surfaceSchemePrefixes` (default `["event", "job"]`, e.g. `event:`/`job:`), THEN THE SYSTEM SHALL return a violation reason string; WHERE `surfaceFormat` is `path`, IF the key contains whitespace or non-path characters AND does not match a configured scheme prefix, THEN it is a violation; WHERE `surfaceFormat` is `any`, no surface format is enforced. Both `http` and `path` accept the same configured scheme prefixes, so a project mixing HTTP routes with scheme-tagged logical keys (e.g. `ui:`) need not fall back to `any` to represent both.
...
- **FR-007** (event): WHEN config is loaded, THE SYSTEM SHALL derive the shared regexes `__specIdRe` and `__coversRe` from `specIdPrefixes`, set `__root` to the config directory (or the start directory when no config file exists), and build `__allVerbs` from CRUD plus `capabilityVerbs` — accepting `capabilityVerbs` as either a legacy array of verb strings or a `{verb: reason}` object, taking the object's keys as the verb set in the latter case.
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | `specSyncExemptGlobs`에 `sdd/FR_INDEX.json` 등재 + `capabilityVerbs`에 `locate` 등록 + `exemptionRegistry`에 그 면제의 분류·사유·영구성 근거 등재(SPEC-062 조회 인덱스·조회 capability) | 생성물이라 소유 스펙 Files의 폐쇄세계 판정에 걸리면 커밋이 막힌다 — `sdd/OWNERSHIP_MAP.md`·`sdd/derivation.json`과 같은 처분. 면제가 곧 무감시는 아니다: 드리프트는 `sdd-sync` R3의 `gen-fr-index --check`가 본다(SPEC-004 배선). `locate`는 SPEC-062가 세운 조회 어휘인데 미등록이라 check-ownership이 매 실행 경고하고 있었다 — 실재하는 capability이므로 등록으로 닫는다(어휘 확장은 그 줄로 계속 표면화된다). 면제 등재는 킷 자신의 도그푸딩 테스트가 즉시 요구했다[실측] — 사유 없는 면제를 금지하는 규범이 실제로 발화한다는 증거 [검증: tooling/__tests__/grammar-hardening.test.mjs, .github/workflows/sdd-gates.yml] |
...
| 2026-08-16 | `riskyActionPatterns` 표면의 설명 주석을 `tool` 필드(도구명 정규식, `match`와 상호 배타) 포함으로 갱신(값 변경 없음, 문서 주석만) | SPEC-058 동반 — MCP 도구 호출(명령 문자열이 없는 도구)은 `match`로 원천적으로 못 잡아 `tool` 필드가 추가됐다 |
...
| 2026-08-10 | `categoryRoleProvenance` + `ROLE_NAME_PATTERNS` 신설 — 역할이 **선언인가 이름 추론인가 미해석인가**를 판정 규칙과 **같은 규칙으로** 계산해 돌려준다(양판) | **추측이 금지인 방법론에서 추측의 성공은 침묵할 수 없다.** FR-010의 이름 정규식 폴백은 **실패할 때만** 사유를 남겼다 — 성공하면 판정이 추측 위에서 진행되고 아무도 모른다. 카테고리를 개명하면(Entities → Aggregates) 조준 대상이 바뀌는데 운영자는 통보받지 못한다. 폴백 패턴을 단일 정본으로 뺀 이유: 출처 계산이 판정과 **다른 규칙**을 쓰면 그 보고 자체가 거짓이 된다(테스트가 두 값의 동치를 고정한다) |
| 2026-08-11 | config 표면에 `frPlacementPolicy`(SPEC-056)·`gateFailureEscalationPolicy`·`gateFailureEscalationThreshold`(기본 3)·`gateFailureLedger`·`gateFailureGuards`(SPEC-057) 추가 + `entityRegistry`에 `fr-placement`·`gate-failure` 등재, Node·Python DEFAULTS 동시 | 실측 제보(다른 프로젝트) — 에이전트가 하루에 같은 실수를 세 번 했다(FR을 섹션 밖에 썼다). 게이트는 매번 잡았지만 "오늘 세 번째"라는 정보가 없었다. `gateFailureLedger`를 null 기본(=`.sdd/gate-failures.jsonl`)으로 둔 이유는 이 킷이 이미 구분해 온 `.sdd/`(세션 로그)와 `sdd/`(채택 선언)의 경계를 그대로 따른 것이다 |
