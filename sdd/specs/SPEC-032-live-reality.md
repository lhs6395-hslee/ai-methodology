...
| 2026-08-09 | **등록 축** 신설 — `liveRealityCoveragePolicy`(off\|advisory(기본)\|hard) + `deployArtifactMarkers` + `liveRealityChecks[].covers` + 실행 축과 정책 분리 + 표준 검사 템플릿 4종(presets) | 실측 제보(2026-08-10 qa에이전트 도입): 배포 산출물 결함 **8건을 배포로 하나씩** 발견했다(ECR 리포 없음·경로 가드·base==HEAD·크로스계정 ECR API·buildx 캐시 드라이버·아치 불일치·리포 정책 부재·이미지에 의존 모듈 누락). 전부 저장소 밖 사실이고 전부 로컬 게이트를 green으로 통과했다. R9라는 **틀은 있었지만** 검사 항목 6건에 이 중 하나도 없었고, **새 산출물을 선언해도 대응 검사를 등록하지 않으면 게이트가 통과**했다 — 틀이 있는 것과 그 틀이 이 산출물을 본다는 것은 다른 사실이다. ⚠ 등록 축이 실행 축과 **정책까지** 갈라져야 하는 이유: 실행은 자격증명·네트워크가 필요해 흔히 off·skipped이고 훅에서 위임되는데, 등록은 순수 선언 대조라 오프라인에서도 판정된다. 한 축에 묶여 있으면 자격증명이 없는 순간 **등록 누락까지 함께 안 보인다** — 그게 제보 (3)의 구조다. `deployArtifactMarkers`는 조용한 기본값을 두지 않는다(SPEC-040 ②): 미선언이면 inert로 자백하고 권장 목록을 지목한다. 담당 선언(`covers`) 없는 검사는 아무것도 커버하지 않는다 — 그걸 커버로 세면 "검사가 하나라도 있으면 통과"가 되고 그게 제보의 상태다. 겸사겸사 양판 갈림도 교정: Node 게이트가 예외 메시지를 stderr로 넘겨 skipped 사유가 Python과 달랐다(코어는 동일했는데 주입값이 갈렸다) [검증: tooling/__tests__/live-reality.test.mjs] |
