...
| 2026-08-10 | **Python 미러 신설**(`cmd_duplicatelogic` + `extract_literals`·`duplicate_literal_findings`·`stale_allow_entries`·`parse_duplicate_candidates`) — 9 시나리오 바이트 패리티 | R12와 같은 결함: 이 축도 Node 전용이라 Python 프로젝트에서 구현 중복을 아무도 보지 않았다. 두 건 모두 **손으로 대조할 때만** 발견 가능한 상태였고, 대응 선언을 기계화한 첫 실행이 즉시 지목했다 — **규범으로만 적힌 의무는 반드시 새어나간다** |
