...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-16 | 선언에 `PreToolUse mcp__github__merge_pull_request\|push_files\|create_pull_request\|update_pull_request_branch\|delete_file sdd-risky-action-check.sh` 추가(선언 6→7종) + 킷 `.claude/settings.json` 재생성 | 실측: 이 매처 층이 `Write\|Edit`·`Bash`뿐이라 MCP 도구 호출(PR 머지 등, SPEC-058)이 통째로 이 층 밖이었다 — 배선 실재를 이 스펙이 판정하는데, 정작 그 배선 자체가 없었다. 명시적으로 위험해 보이는 도구만 나열했다(오너의 명시적 선택 — 모든 MCP 도구를 광범위 매처로 덮지 않는다) [검증: tooling/__tests__/agent-wiring.test.mjs] |
| 2026-08-10 | 선언에 `PreToolUse Bash sdd-diagnosis-check.sh` 추가(선언 4→5종) + 킷 `.claude/settings.json` 재생성 + 선언 개수를 박던 테스트를 **선언 파일 우선** 단언으로 교체 | SPEC-053 동반 — 진단 가드가 이 층에 살고 그 **배선 실재는 이 스펙이 판정한다**(층 합성). 선언을 늘리자 R19가 즉시 5종을 재확인했다. 테스트에서 개수 단정을 뺀 이유: 훅을 추가할 때 그 테스트가 먼저 깨지면 "숫자만 고치기"가 정상 경로가 되고, 그러면 테스트가 선언 파일이 아니라 자기 숫자를 지키게 된다 [검증: tooling/__tests__/agent-wiring.test.mjs] |
...
| 2026-08-10 | `agentWiringFindings`가 스크립트 실재를 3상태로 판정해 `unchecked` 통로를 갖고, 게이트가 그것을 출력한다(SPEC-054) | 실측: 읽기 실패가 `false`로 붕괴하면 "배선된 스크립트가 없다"는 거짓 위반이 된다. 그리고 코어가 통로를 가져도 게이트가 출력하지 않으면 **조용한 0건**이라 결함이 그대로다 — 계약은 코어와 게이트 양쪽에서 성립한다 |
| 2026-08-14 | 선언에 `PreToolUse Bash sdd-risky-action-check.sh` 추가(선언 5→6종) + 킷 `.claude/settings.json` 재생성 | SPEC-058 동반 — 위험 행동 승인 게이트가 이 층에 살고 그 배선 실재는 이 스펙이 판정한다(층 합성, SPEC-053 동반 선례와 동일 논리). 선언을 늘리자 R19가 즉시 6종을 재확인했다 |
