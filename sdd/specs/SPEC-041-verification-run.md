...
- **한 원장에 두 종류가 산다 — 자산 기록(`asset`)과 분기 발화 기록(`branch`, SPEC-049).** 원장을 둘로 나누면 한쪽만 갱신돼 두 회계가 갈라지므로 같은 파일에 담고, 각 파서는 **상대 종류를 조용히 건너뛴다**. 실측: 이 구분 없이 분기 기록을 넣자 자산 축이 그것을 "asset 없음"으로 세어 `hard`에서 거짓 차단이 났다.
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-10 | 원장에 **분기 발화 기록** 종류 추가(SPEC-049) — `--record-branch` 기록 모드 + 회계 축 배선, 자산 파서가 분기 기록을 조용히 건너뛰도록 종류 판별 명시(양판) | 제보 사례 6: 명세·구현·단위테스트가 모두 정상인데 배선이 없어 비교가 단 한 번도 수행되지 않았고, 정적 검사로는 원리상 잡히지 않는 층이다. 자산 실행 회계의 다음 질문이 분기 발화이므로 **같은 원장**에 담는다 — 나누면 두 회계가 갈라진다. 종류 판별을 명시하지 않았을 때 자산 축이 분기 기록을 깨진 기록으로 세어 거짓 차단이 난 것을 실측으로 확인하고 고쳤다 [검증: tooling/__tests__/verification-run.test.mjs] |
