...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-27 | `tooling/harness/self/pre-commit`의 강도 안내 주석을 `capabilityVerbPolicy` 신설(SPEC-002 FR-011)에 맞춰 갱신 + `self-hooks-install.sh` 재실행으로 킷 자신의 `.git/hooks/pre-commit` 사본을 원본과 재동기화 | 감사 이슈 #21 문서-코드 드리프트 정정 — 이 스펙이 원본↔사본 신선도 축(FR-005)을 판정하므로, 원본 주석이 바뀌면 사본도 즉시 재동기화해야 이 스펙 자신이 `stale`을 자백하지 않는다(도그푸딩). 판정 로직 무변경 |
...
| 2026-08-11 | `tooling/harness/self/commit-msg`(킷 자기호스팅 원본)에 `tooling/check-fr-placement.mjs`·`tooling/check-gate-escalation.mjs`를 `check-spec-sync.mjs` 뒤에 순차 체인 | SPEC-056·057 동반. 킷 자신도 자기 궤도 위에서 돌아야 하므로(자기 정렬) 소비 프로젝트용 `tooling/harness/commit-msg`와 같은 순서를 킷 자신의 훅에도 그대로 반영했다 |
