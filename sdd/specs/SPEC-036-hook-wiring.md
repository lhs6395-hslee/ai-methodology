...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-17 | `tooling/harness/self/commit-msg`(킷 자기적용 훅)에 `check-duplicate-source-drift.mjs`·`check-invariant-guard.mjs`(SPEC-059, R26) 호출 단계 추가(`check-gate-escalation.mjs` 뒤) | 킷 자신도 이 신규 게이트 무결성 감사 축을 도그푸딩해야 한다 — consumer 템플릿(`tooling/harness/commit-msg`, SPEC-003)만 고치고 자기적용 훅을 빠뜨리면 킷 자신에서는 그 축이 한 번도 발동하지 않는다 |
