...
| 2026-07-28 | 대상(new) 키 실재 검증의 선언 FR 수집을 SPEC-013 FR-008 단일 범위 판정으로 교체 | retag가 전문 스캔으로 "실재"를 인정하면 Change Log에만 굵게 적힌 번호를 유효 대상으로 통과시키고, 그 태그를 fr 게이트 R1이 dangling으로 되쏘는 불일치가 생긴다(선언 정의를 게이트와 일치시켜 봉합) |
