...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-17 | 순차 프로세스 SSOT(SPEC-047 FR-005, 불변식 강제 여부 4번째 축)의 Python 미러(`invariant_of`·`invariants_of`·`validate_invariants`·`ssot_missing_invariants`·`unenforced_invariant_findings`·`stale_enforcement_mention_findings` + `cmd_processssot` 확장) + 패리티 시나리오 5건 추가(processssot 테스트에 통합) | SPEC-047은 이미 이 스윗의 패리티 테스트 대상(SC-002)이라 4번째 축 추가 시 미러 갱신이 함께여야 바이트 동일이 유지된다. 작성 중 Windows 전용 테스트 하네스 결함 2건을 같은 파일(sdd-gates-py.test.mjs)에서 발견·수정했다 — ① 픽스처 헬퍼가 `new URL(...).pathname`으로 소스 경로를 구했는데 Windows에서 드라이브 문자 앞에 슬래시가 남아 모든 e2e/패리티 테스트가 ENOENT로 실패하고 있었다(`fileURLToPath`로 교체) ② `runPy()`가 Windows 콘솔 기본 코드페이지(cp949)로 죽는 것과(`PYTHONIOENCODING=utf-8` 추가) Python 텍스트모드 stdout의 `\n`→`\r\n` 변환을 대조 전 정규화하지 않는 것(테스트 하네스에서만 정규화, sdd_gates.py 자체는 무변경) — 이 결함들 때문에 이 테스트 스윗의 패리티 검증이 이 플랫폼에서 한 번도 성립한 적이 없었다(전수 실행: 개선 전 fail 294/955, 개선 후 fail 236/968 — 새 테스트 13건 추가 순증분을 빼면 순수 수정으로 58건 감소) [검증: tooling/__tests__/sdd-gates-py.test.mjs] |
