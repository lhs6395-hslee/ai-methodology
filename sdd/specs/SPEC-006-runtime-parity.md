...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | `symbol_candidates` 독스트링 정정 — "점 표기를 경로와 basename으로도 본다"에서 basename 문구 삭제하고 그것을 **의도적으로 뺀 이유**(느슨한 후보가 아무 위치의 동명 파일이나 통과시킨다)를 Node판 주석과 같게 명시. 코드 무변경 | 문서-코드 드리프트[실측]: 코드는 점→`/` 변환만 하고 basename은 후보에 넣지 않는데(Node판도 같고, 그 주석은 이유까지 적어 뒀다) Python 독스트링만 넣는다고 적혀 있었다. 패리티 스펙의 미러 함수에서 설명이 갈라지면 다음 사람이 "Python판이 더 관대하다"고 오독하고 그 오독대로 한쪽을 고친다 [검증: tooling/__tests__/sdd-gates-py.test.mjs] |
| 2026-08-27 | `spec-template.md`를 정식 `templates/module-spec.md`와 재동기(1 aggregate root 경계 주석을 `specCohesionPolicy`/`supportLayerSpecs`/`maxAggregateRootsPerSpec` 예외 메커니즘 언급으로 교정, 바이트 동일 유지) | 이슈 #21 M-9: `templates/module-spec.md` 개정에 preset 사본이 따라오지 않으면 `template-parity.test.mjs`가 드리프트를 잡는다(설계대로) — 두 사본을 함께 갱신 |
| 2026-08-16 | 위험 행동 승인(SPEC-058)의 `tool` 필드 확장을 Python 미러(`_sort_keys_deep`·`canonical_tool_payload`·`tool_call_from_hook_input` 신설, `parse_risky_action_patterns`/`validate_risky_action_patterns`/`match_risky_action`/`cmd_riskyaction` 갱신) + MCP 도구 훅·tool 스윕 결함 3종 패리티 신규 시나리오 | `canonical_tool_payload`는 해시 대상이라 Node의 `JSON.stringify`와 바이트 동일해야 한다 — Python `json.dumps` 기본값(공백 구분자·`ensure_ascii=True`)은 그대로 두면 갈리므로 `separators=(",",":")`·`ensure_ascii=False`를 명시했다. 키 정렬은 `input`에만 적용하고 바깥 `{tool, input}` 순서는 삽입 순서를 유지해 Node의 객체 리터럴 순서와 맞췄다(sort_keys=True를 전역으로 걸면 `input`이 `tool`보다 알파벳순으로 앞서 순서가 갈린다) [검증: tooling/__tests__/sdd-gates-py.test.mjs] |
...
| 2026-08-11 | Python 미러 — `fr-placement-lib.mjs`/`check-fr-placement.mjs`(`cmd_frplacement`)와 `gate-failure-lib.mjs`/`check-gate-escalation.mjs`(`cmd_gateescalation`) 전체 이식 + `verdict()`/`arm_verdict`에 `meta`·원장 append·`_EXIT_CODE` 캡처 확장(양판) + 래칫 목록 패리티 테스트(⑥)가 잡은 `preEditSpecFirstPolicy` Python 누락 수정 | SPEC-056·057 동반. 이식 중 실측 결함 둘: (1) 단일 파일 안에서 `GUARD_FINDING_TEXT`라는 같은 이름의 딕셔너리가 이미 존재해(SPEC-053 진단 가드) 뒤 정의가 앞 정의를 조용히 덮어 새 게이트가 `KeyError`로 죽었다 — Node는 파일마다 스코프가 있어 이 충돌이 없는데 단일 파일 미러에서만 재현된다. `GATE_FAILURE_GUARD_FINDING_TEXT`로 개명해 해소. (2) 새 패리티 테스트(⑥)의 정규식 추출기 자체가 `[A-Za-z]+`라 숫자 포함 knob(`e2eTestsPolicy`)을 놓쳐 거짓 드리프트를 보고했다 — 실제 드리프트가 아니라 테스트의 결함이라 `[A-Za-z0-9]+`로 교정 [검증: tooling/__tests__/sdd-gates-py.test.mjs] |
