...
| 2026-08-16 | `tooling/harness/self-adoption-install.sh` 신설(Files 편입) — 킷 자기적용판 `sdd/adoption.json`을 `tooling/` 스캔으로 재생성한다 | 실측: 킷 자신의 `sdd/adoption.json`이 kitCommit 37dba3c(2026-08-10)에 멈춰 R23~R25 게이트가 선언에 없었다. 소비 프로젝트는 `sdd-init.sh` 재실행이 이 파일을 자동 갱신하지만, 이 레포는 `scripts/`가 없어(원본 직접 호출, self-hooks-install.sh와 같은 이유) 그 경로를 못 탄다. R17은 "선언된 게이트가 실재하는가"만 보고 "지금 있는 모든 게이트가 선언에 있는가"는 안 봐서 이 뒤처짐을 스스로 못 잡았다 [검증: sdd/adoption.json] |
