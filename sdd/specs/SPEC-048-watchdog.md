...
- **마커가 라벨에 걸린 것은 호출이 아니다** — 실측: 킷 워크플로 파일명이 `sdd-gates.yml`이고 안에 `name: sdd-gates`가 있어서 이 게이트가 **자기 파일명에 매치해** 여러 달 "스윕이 배선돼 있다"를 보고했다. 감시자 실재를 판정하는 게이트가 자기 이름에 속았다.
- **데이터 파일 경로도 호출이 아니다** — 선행 문맥(`/`·러너)만 보면 `--record ".github/workflows/sdd-gates.yml"` 같은 **인자 안의 경로**가 호출로 읽힌다. 실행 확장자(`.mjs`·`.py`·`.sh`)이거나 확장자가 없어야 호출이다 — `.yml`은 읽히는 것이지 도는 것이 아니다.
- **아예 없는 것과 라벨에만 있는 것은 다르다** — 없으면 이 게이트가 고발하지만, 라벨에 걸리면 **아무도 고발하지 않는다.** 후자가 더 나쁘므로 이름을 댄다.
- **`--strict` 없는 스윕 호출은 채널이 아니라 로그다** — advisory 발견에서 exit 0으로 끝난다. 우회 불가한 자리에서 통과만 하면 우회할 필요도 없다.
- **CI가 게이트를 손으로 열거하면 빠진 게이트는 어떤 우회 불가 층에도 없다** — 실측: 스윕 등재 25종 중 10종이 CI 목록에 없었고 그중에 이 게이트 자신이 있었다. 이전 판은 손목록 위에 "게이트를 추가하면 여기에도 추가한다"는 주석을 달았고 **그 규범은 실패했다.**
- **스윕 규칙표를 못 찾으면 강제 층 커버리지를 판정하지 않는다** — 0종으로 세면 "전부 덮였다"는 거짓 초록이 된다(모르는 것을 숫자로 말하지 않는다).
- **차단했다는 것과 옳은 이유로 차단했다는 것은 다른 사실이다** — 비-0 종료만 단언하면 게이트가 엉뚱한 이유로 죽어도(정책 enum 위반·크래시·경로 오류) 테스트는 초록이고 심어둔 위반은 놓친 채로 남는다. 이 계열 결함은 실패가 아니라 **통과**로 나타난다. 그래서 차단을 단언하는 테스트는 **게이트 출력도 대조해야** 한다.
---
...
- **FR-005** (event): WHEN a continuous-integration file is examined for a sweep invocation, the **watchdog** (E) core SHALL count a marker occurrence as an invocation only when it is preceded by a runner token or a path separator and is not followed by a non-executable file extension, SHALL report a marker that appears only on a label line as a distinct violation rather than as an absent channel, and SHALL report an invocation that cannot exit non-zero as a channel that only logs.
- **FR-006** (unwanted): IF the continuous-integration configuration enumerates gates by hand instead of invoking the sweep, THEN the gate SHALL name every sweep-registered gate absent from that enumeration as reachable from no unbypassable layer, and SHALL decline to judge this at all when the sweep rule table cannot be located, because counting zero registered gates would report full coverage.
...
- **Files**: tooling/watchdog-lib.mjs, tooling/check-watchdog.mjs, tooling/__tests__/watchdog.test.mjs, sdd/adoption.json, tooling/harness/self-adoption-install.sh
...
- **SC-005**: 킷 자신의 CI가 게이트를 손으로 열거하지 않는다 — 스윕 호출 1줄이고, 강제 층 밖 게이트 0종이다(도입 시 10종 검출). [검증: tooling/__tests__/watchdog.test.mjs]
- **SC-006**: 차단을 단언하는 게이트 테스트가 **출력까지 대조**한다 — 스윕 등재 게이트 전수(측정 시점 27/27). [검증: tooling/__tests__/ship-closure.test.mjs]
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-10 | 채택 영수증에 `agentHooks`·`agentSettings` 기록(킷 자기적용판 갱신) + 감시자 축의 **경계 명문화**: 이 축은 커밋 이후 채널(CI)과 영수증을 보고, 에이전트가 도구를 쓰는 순간의 배선은 SPEC-051(R19)이 본다 | 오너 실측이 이 스펙의 사각을 지목했다 — **R17이 초록인 동안 감시 에이전트가 전무했다.** R17은 "우회 불가한 채널이 있는가"를 정확히 답하고 있었고 거짓말한 것이 아니다: 문제는 그 답이 "에이전트가 지금 감시되는가"로 읽혔다는 것이다. 영수증이 git 훅만 기록한 것도 같은 사각의 일부였다 — 감시자가 무엇을 깔았는지가 영수증의 존재 이유인데 에이전트 훅은 거기 없었다 [검증: tooling/__tests__/agent-wiring.test.mjs] |
...
| 2026-08-10 | `missingGates`가 3분류 계약(SPEC-054)을 따라 `{gone, unchecked}`를 돌려주고 게이트가 확인 못 한 게이트를 **표면화하되 "지워졌다"로 단정하지 않는다** + 픽스처 복사 목록 폐포 계산 | 실측: 감시자 실재 판정에서 읽기 실패가 `false`로 붕괴하면 "감시자가 지워졌다"는 거짓 경보가 된다. 거짓 경보가 잦은 감시자는 꺼진다 |
| 2026-08-10 | FR-005·FR-006·SC-005 신설 — **거짓 초록 수정**: 스윕 호출 문법화(선행 러너·경로 + 실행 확장자), 라벨 전용 매치를 별도 위반으로, `--strict` 없는 호출을 "로그일 뿐"으로, CI 손목록에서 빠진 스윕 게이트 열거(`sweepInvocation`·`sweepBlocking`·`sweepGateFiles`·`gatesOutsideCi`) | **감시자 실재를 판정하는 게이트가 자기 파일명에 속았다.** 마커 목록에 `sdd-gates`가 있고 킷 워크플로가 `sdd-gates.yml`(안에 `name: sdd-gates`)여서 여러 달 "✓ 스윕이 배선돼 있다"를 보고했는데 **CI는 스윕을 한 번도 부르지 않았다.** 그 사이 스윕 등재 25종 중 **10종이 어떤 우회 불가 층에도 없었다** — R13·R15·R16·R18·R19·R20·R21·R5·R12, 그리고 **이 게이트 자신.** 선행 문맥만 고치면 두 번째 거짓 양성이 남았다: `--record ".github/workflows/sdd-gates.yml"`의 **인자 안 경로**가 `/` 선행으로 매치했다 — **데이터 파일은 프로그램이 아니다.** "라벨에만 있다"를 별도 위반으로 뽑은 이유: 아예 없으면 이 게이트가 고발하지만 라벨에 걸리면 **아무도 고발하지 않는다** [검증: tooling/__tests__/watchdog.test.mjs] |
| 2026-08-10 | 스윕 규칙표에 **게이트↔Python 서브커맨드 대응 선언**(`PY_SUBCOMMAND`) 추가 | 스윕은 "무엇을 도는가"의 정본이므로 "그것을 다른 런타임에서 무엇이 도는가"도 같은 자리에 산다. 두 곳에 적으면 한쪽이 뒤처지고, 이 파일이 존재하는 이유가 바로 그것이다 |
| 2026-08-10 | 카나리아 계약 강화 — 차단 단언에 **출력 대조**를 함께 요구한다(SC-006) | 오너 검토 요청: "심어둔 위반을 게이트가 놓치면 그 실행을 실패로 처리하는 카나리아 층". 측정 결과 이미 27/27이 충족하고 있었으므로 **새 강제를 만드는 것이 아니라 현재 상태를 되돌아가지 못하게 못박는 것**이 맞는 조치였다(래칫과 같은 기조). 비-0 종료만 단언하면 게이트가 정책 enum 위반·크래시 같은 **엉뚱한 이유로 죽어도** 테스트가 초록이고, 심어둔 위반은 놓친 채로 남는다 — 이 계열 결함은 실패가 아니라 **통과**로 나타난다는 제보의 논거 그대로다. 앞선 라운드에 내가 "48개 차단 단언 중 14개가 건수를 고정하지 않는다"고 보고한 것은 **개별 단언 지점** 기준이었고, 게이트 단위로는 27/27이 사유를 고정하고 있었다 — 같은 사실을 다른 단위로 세면 다른 숫자가 나온다 [검증: tooling/__tests__/ship-closure.test.mjs] |
| 2026-08-14 | `tooling/harness/self-adoption-install.sh` 신설 — 킷 자기적용판 `sdd/adoption.json`을 `tooling/`(소비 프로젝트의 `scripts/`가 아니라) 직접 스캔해 재생성 | 오너 지적("하네스가 제대로 돈 적이 없다")을 실측으로 검증하던 중 발견: 이 레포 자신의 `sdd/adoption.json`이 `kitCommit 37dba3c`(2026-08-10)에 멈춰 있었고 그 뒤 추가한 R23·R24·R25 게이트가 선언에 없었다. 소비 프로젝트는 `sdd-init.sh` 재실행이 이 파일을 자동 갱신하지만, 이 레포엔 `scripts/`가 없어(자기적용은 `tooling/`을 직접 호출) 그 경로를 못 탄다 — `self-hooks-install.sh`가 git 훅에 대해 이미 갖고 있던 것과 같은 자기적용 재생성 경로가 채택 영수증에는 없었다. R17은 "선언된 게이트가 실재하는가"만 보고 "지금 있는 모든 게이트가 선언에 있는가"는 안 봐서 이 뒤처짐 자체를 못 잡는다(계속 초록이었다) — 그래서 검출이 아니라 **재생성 스크립트**로 해소했다(오너 결정: R17 양방향 강화 대신 자동 재생성). 실행해 30→39종으로 갱신 확인 |
