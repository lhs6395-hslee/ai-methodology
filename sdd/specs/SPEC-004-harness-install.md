...
| 2026-08-14 | `sdd-init.sh` 완료 안내에 "미선언 축" 체크리스트 추가 — `riskyActionPatterns`(R25)·`diagnosisSpecMap`(R21)·`surfaceGlobs`(R2)가 비어 있으면(키 부재 또는 `[]`) 설치·업데이트 직후 비대화형으로 알려준다(Node로 실제 JSON 파싱, node 부재 시 조용히 건너뜀) | 오너 지적: "게이트나 훅이 설정 안 되는 것 같다 — 최초 설치·업데이트 때 반영할지 물어보는 거라도 있으면 좋겠다." 확인 결과 훅 배선 자체는 자동이지만, 이 3축처럼 "무엇을 위험/진단/표면으로 볼지"는 프로젝트가 선언해야 하고 미선언이면 **훅 모드에서 완전히 침묵**한다(설계상 정상 — 소음 방지) — 그런데 이 사실이 완료 안내 어디에도 없어 사람이 스윕을 직접 돌려 산문을 읽기 전까진 아무도 몰랐다. 오너 확정: 비대화형 안내문(CI 등 비대화형 설치에서도 안전) + 선언-의존 INERT 축 전체(범위를 R25 하나로 좁히지 않음) [검증: 설치·재실행(선언 후) 양쪽 시나리오 수동 검증, node --test 967 pass] |
