...
- PreToolUse `sdd-edit-check.sh`는 **config `scanDirs`가 코드로 규정한 경로** 편집에만 체크리스트를 상기하고 그 외에는 침묵한다. ⚠ 이전 판의 `src`/`lib`/`app` 하드코딩은 제거됐다 — 실측: 킷의 `scanDirs`가 `tooling`이라 그 체크리스트가 **한 번도 발화할 수 없었다**(SPEC-051).
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-09-04 | FR-002 개정 + `sdd-init.sh` 2e 단계 신설 — 설치·업데이트 끝에 스펙이 있으면 `gen-fr-index.mjs`를 한 번 돌려 FR 조회 인덱스를 프라이밍한다(스펙 0건이면 안내만, 생성 실패는 경고이고 설치를 막지 않는다) | SPEC-062 인덱스는 "있으면 빠른" 캐시라서 안내만 두면 아무도 만들지 않는다 — 그러면 조회가 계속 스펙 전수 읽기로 돌면서 **느려진 사실이 아무에게도 보이지 않는다**. `/sdd-update`가 `sdd-init` 재실행이므로 이 한 단계로 소비 프로젝트가 업데이트마다 최신 인덱스를 갖는다(오너 요구인 O(1) 조회가 킷에만 남지 않게 하는 배선) [검증: tooling/__tests__/init-gates.test.mjs] |
| 2026-09-04 | `sdd-session-context.sh` 진입 규칙에 1b(FR 조회) 추가 — "어느 FR인지는 `sdd-where`로 조회하고 통독하지 말 것" + 낡은 인덱스 재생성 명령 | 조회기·인덱스를 만들어도 **부르는 자리가 없으면 안 쓰인다** — 이 킷이 반복해 잡는 결함 클래스(선언은 있고 호출처 0곳)를 자기 축에도 적용했다. 편집 시점은 pre-edit 훅이 이미 띄우지만 "새 요구사항 추가·기존 갱신"처럼 편집 **전** 단계에는 강제점이 없어 세션이 통독으로 되돌아갔다(오너 제보의 실제 상황) [검증: tooling/__tests__/hooks-install.test.mjs] |
| 2026-09-04 | `sdd-init.sh` 배포 목록에 `gen-fr-index.mjs` 추가 + `sdd-sync.mjs` R3에 `gen-fr-index.mjs --check --if-present` 배선(SPEC-062, `notAJudge` 등재로 양판 대상 제외) | 조회 인덱스 생성기가 킷에만 있으면 소비 프로젝트는 인덱스를 못 만들어 조회가 계속 스펙 전수 읽기로 폴백한다 — `gen-ownership-map.mjs`와 같은 처분(소비 프로젝트 산출물). 드리프트 강제점이 없으면 인덱스가 조용히 낡아 폴백만 남으므로 스윕에 `--check`를 걸었다(미생성은 실패 아님 — 선택 산출물) [검증: tooling/__tests__/ship-closure.test.mjs, tooling/__tests__/fr-index.test.mjs] |
...
| 2026-08-10 | 스윕 규칙표에 게이트↔Python 서브커맨드 대응 선언(`PY_SUBCOMMAND`) 편입 + 생성 변경 이력 재생성 | 스윕은 "무엇을 도는가"의 정본이므로 대응 선언도 같은 자리에 산다 — 두 곳에 적으면 한쪽이 뒤처진다 |
| 2026-08-10 | 스윕 R22 등재 + `PY_SUBCOMMAND`·설치기 복사 목록에 완료 신호 게이트 편입 + `--json` 규칙 id 기대 목록 갱신 | 규칙을 늘리면 스윕 등재·배포 폐포·양판 대응·기계 리포트 네 곳이 함께 움직여야 하고, 그중 하나라도 빠지면 그 게이트는 어딘가에서 조용히 사라진다. 넷 다 게이트가 검산한다(손목록이 아니다) |
| 2026-08-10 | `self-hooks-install.sh`가 heredoc 대신 `harness/self/`의 훅 본문 파일을 복사한다 + 생성 변경 이력 재생성 | 설치기 안 본문은 **대조할 원본이 없다** — 그래서 R12의 신선도 축이 킷 자신에게 영구히 미판정이었다. 설치기가 파일을 복사하면 그 파일이 정본이 되고 낡은 사본이 `stale`로 잡힌다 |
| 2026-08-10 | 배포 폐포 카나리아 계약(`ship-closure.test.mjs`)에 **출력 대조 요구** 추가 + 생성 변경 이력 재생성 | 차단 단언만으로는 게이트가 엉뚱한 이유로 죽은 경우와 심어둔 위반을 잡은 경우를 구분하지 못한다 — 측정 시점 27/27이 이미 충족하므로 이 계약은 **현재 상태를 되돌아가지 못하게 못박는 것**이다(래칫과 같은 기조) |
| 2026-08-10 | `sdd-edit-check.sh`가 게이트 종료코드를 **전파**한다 + `hard`인데 판정 런타임이 없으면 거짓 안전을 자백한다 | 이전 판은 `2>/dev/null` + 무조건 `exit 0`으로 종료코드를 삼켰다 — 정책이 `hard`여도 편집이 막히지 않았고 강도 사다리의 종착지가 **도달 불가**였다. 그리고 node가 없는 프로젝트가 `hard`를 켜면 강제가 켜졌다고 믿으면서 보호가 0이다: 막지는 않되(런타임 부재로 편집을 멈추면 작업이 통째로 죽는다) 매 발동 그 사실을 말한다 — **hard 선언 + 무판정 = 거짓 안전** |
| 2026-08-10 | `sdd-edit-check.sh`가 판정 런타임을 **선택**한다 + 둘 다 없을 때만 "검사 못 함"을 낸다 | 이전 판은 Node 게이트만 알았다. 하네스가 런타임 중립이라는 이 스펙의 전제가 이 훅에서만 깨져 있었다 |
| 2026-08-11 | `sdd-init.sh`의 Node 게이트 배포 목록에 `fr-placement-lib.mjs`·`check-fr-placement.mjs`·`gate-failure-lib.mjs`·`check-gate-escalation.mjs` 4파일 추가 + Python 분기 commit-msg 훅 template에 `frplacement`·`gateescalation` 서브커맨드를 `specsync` 뒤에 체인 + 스윕 규칙표에 R23·R24 등재(`sdd-sync.mjs`) | SPEC-056·057 동반. 배포 목록 누락은 이 킷에서 세 번 재발한 형태(verdict-lib·verification-run-lib·term-coverage-lib)라 계약 테스트가 이미 강제하고 있고, 그 테스트가 이번에도 누락을 즉시 잡았다 |
| 2026-08-13 | `agent-context.md`의 진입 규칙 5를 **작업 유형별 두 갈래**로 개정 — 신규 기능(spec-first: FR 확정→코드→테스트)과 버그 수정(재현 RED 먼저→코드→GREEN→스펙 갱신, `/speckit.fix`)을 분리하고, 공통 규칙(스펙·코드는 같은 changeset, 디스크 작성 순서 무관)을 규칙 6에 명시. `speckit-fix.SKILL.md` 서두에 이 분기를 가리키는 교차 참조 한 단락 추가 | 오너 제보: 기존 문서는 "spec-first"만 단일 규칙으로 적혀 있어 버그 수정에도 스펙부터 쓰라는 것으로 오독됐고, 실제로 재현 전에 추측성 스펙 문구부터 쓴 뒤 원인이 다른 곳으로 드러난 사례가 있었다(찔러보기 금지 원칙과 충돌). `/speckit.fix` 스킬은 이미 재현→코드→스펙 순서를 따르고 있었으므로 그 실전 순서를 상시 로드 문서에 명문화했다 — 두 절차는 이미 있었고 이번 변경은 **어느 문서에도 그 구분이 드러나지 않아 오독을 유발하던** 상시 로드 문서의 표현만 고쳤다 |
| 2026-08-14 | `sdd-session-context.sh`의 SessionStart 배너가 여전히 옛 단일 규칙("코드 전에 spec부터")을 하드코딩하고 있던 것을 발견해 `agent-context.md` 규칙 5와 같은 두 갈래로 재동기화 | 오너 확인 질문("update 절차가 정말 제대로 동작하는가")에 답하려고 실제로 뒤져보다 발견 — `agent-context.md`(2026-08-13 개정, 위 행)를 고치면서 **같은 문구를 하드코딩으로 들고 있는 이 파일을 놓쳤다.** 이 파일은 Claude Code SessionStart 훅이 매 세션 첫머리에 주입하는 배너라 agent-context.md보다 먼저·더 자주 읽히는데, 정확히 이 킷이 다른 곳에서 경고해 온 "규범이 복사되면 잊힌다" 패턴 그 자체였다 |
