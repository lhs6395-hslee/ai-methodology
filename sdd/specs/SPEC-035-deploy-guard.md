...
- Node 전용이다. 런타임 패리티 대상은 **판정 게이트**이고, 훅 헬퍼가 없는 런타임에서도 커밋·CI 강제는 그대로 작동한다. ⚠ 선례로 들던 `check-pre-edit`은 2026-08-10부터 **차단 강도를 갖는다**(SPEC-003 FR-001) — 더는 "편의 계층" 선례가 아니므로 그 인용을 걷어냈다. 차단할 수 있는 층을 편의 계층이라 부르면 같은 대상에 두 스펙이 반대 성격을 부여하고, 그것이 R20이 잡는 모순이다.
