...
| 2026-08-14 | `verdict.test.mjs`의 `runSnippet()`이 스폰하는 자식 프로세스에 `cwd`를 격리된 tmp 루트로 고정 | 실측: `cwd`를 지정하지 않아 `appendGateFailure`의 `resolveLedgerPath()`가 `process.cwd()`(=테스트를 실행한 실제 저장소)부터 위로 `sdd.config.json`을 찾았고, 그 결과 이 테스트의 가짜 판정(`gate:"g.mjs"`, 위반 3건)이 **킷 자신의 진짜 원장**(`.sdd/gate-failures.jsonl`)에 반복해 새어들었다(2026-08-11~08-14, 37건 확인). "반복이 기억되는가"를 보장해야 할 축이 자기 테스트 때문에 거짓 기억을 쌓고 있었다 — 정확히 이 킷이 경계하는 종류의 결함이다(오염된 로컬 원장 37건은 함께 정리, `.sdd/`는 gitignore라 커밋 이력엔 없었다) [검증: tooling/__tests__/verdict.test.mjs] |
