...
- **NFR-001**: 판정 코어는 문자열 파싱·집합 대조만의 순수 함수라 결정적으로 단위 테스트되고, 자산 실재 판정(파일 IO·글롭)은 소비 게이트가 주입한다. [검증: tooling/__tests__/evidence.test.mjs]
...
| 2026-08-09 | 증거 **등급 분리** — `deploy-needs-live-evidence` 신설(`deployEvidencePatterns`·`deployMarkers`, 브라우저 등급 동형) | 실측 제보 (4): "단위테스트 통과"와 "배포본에서 실제 실행됨"을 같은 증거로 취급하면, 아치 불일치(arm64 노드에 amd64 이미지)·이미지 안 모듈 누락처럼 **런타임에서만 깨지는 결함**이 100% green 위에서 배포된다. 저장소 안 테스트는 배포본의 아치·이미지 내용·전제 자원에 닿지 않는다. ⚠ 트리거는 **소유 + 주장 둘 다**여야 한다 — 마커만으로 걸었더니 킷 시운전에서 31건이 쏟아졌다(배포 가드의 *판정 로직*처럼 배포를 다루기만 하는 스펙까지 잡힌다). 그런 스펙의 정답 증거는 단위테스트가 맞다: **소유 없는 마커는 화제이지 대상이 아니다.** 소유 판정은 SPEC-032의 `deployArtifactMarkers`를 **재사용**한다(같은 사실에 목록이 둘이면 갈라진다). 범위: 아치 불일치·모듈 누락은 제보 프로젝트의 컨테이너 배포 파이프라인에서 관측된 것이고, 여기서 끌어낸 결론은 "저장소 안 테스트는 배포본 런타임에 닿지 않는다"는 일반 명제까지다 — 어떤 경로 패턴이 배포 등급인지는 프로젝트마다 다르므로 `deployEvidencePatterns`로 교체한다 [검증: tooling/__tests__/evidence.test.mjs] |
| 2026-08-10 | `evidenceFindings`가 자산 실재를 3상태로 판정하고(`asset-unchecked`) 게이트가 그것을 **차단하지 않되 초록에도 합산하지 않는다**(SPEC-054) | 실측: 존재 판정기가 예외를 `false`로 삼키면 읽기 실패가 "증거 자산 없음"이라는 거짓 위반이 된다. 반대로 통과로 흘리면 증거 없는 주장이 초록이 된다 — 두 방향 모두 모르는 것을 아는 것처럼 말한 것이다 |
