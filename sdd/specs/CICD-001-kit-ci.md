...
| 2026-08-10 | 게이트 손목록(18종)을 **스윕 호출 한 줄**로 교체(`node tooling/sdd-sync.mjs --strict`) | 이전 판은 손목록 위에 "게이트를 추가하면 여기에도 추가한다"는 주석을 달았고 **그 규범은 실패했다** — 스윕 등재 25종 중 10종이 이 파일에 없었고 그중에 감시자 게이트 자신이 있었다. 해소는 목록을 고치는 것이 아니라 **목록을 없애는 것**이다: 스윕이 게이트 목록의 정본이므로 게이트를 추가해도 이 파일은 바뀌지 않는다 — **바뀌지 않는 것이 요점이다**(바뀌어야 하는 목록은 반드시 뒤처진다). `--strict`를 붙인 이유: 없으면 advisory 발견에서 exit 0으로 끝나고, 우회 불가한 자리에서 통과만 하는 채널은 채널이 아니라 로그다 |
