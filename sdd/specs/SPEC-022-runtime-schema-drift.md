...
| 2026-08-10 | 로컬 `isMainEntry` 정의를 제거하고 `verdict-lib`의 공용 정의를 쓴다(판정 동일 — 정의 위치만 이동) | SPEC-021과 같은 사본 정리. 같은 규칙이 세 파일에 구현돼 있으면 한쪽만 고쳐지고, 실제로 그 다음 라운드의 새 게이트들이 깨진 형태로 태어났다 [검증: tooling/__tests__/import-wiring.test.mjs] |
