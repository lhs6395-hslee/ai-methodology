...
**Input**: `STRUCTURE.md` §3.1의 "1 spec = 1 aggregate" 원칙은 실사용에서 자주 무너진다(도그푸딩: 소비 프로젝트 B의 SPEC-005가 aggregate root 7개로 cohesion 상한을 넘김). 과대 spec을 aggregate별로 쪼갤 때, 쪼개진 Entity들 사이의 관계를 표현할 문법이 없었다 — `## Dependencies`의 `Entities:` 라인은 완전한 자유 텍스트라 대상의 실재·소유 spec·순환 참조를 게이트가 전혀 검사하지 못했다(`check-ownership.mjs`가 파싱은 하되 `_deps`라는 이름으로 명시적으로 버림). `EntityName (relation-type)` 괄호 표기만 구조화된 관계로 인정해, 대상 Entity의 실재·소유 spec 해석은 hard로, aggregate 간 순환 참조는 advisory로 검사한다.
...
| 2026-07-28 | 관계 판정이 **한 번도 발화하지 않은 상태**를 표면화 — Dependencies에 참조는 있는데 전부 자유참조(타입 없음)면 대상 실재 검증(FR-002)·순환 탐지가 아무것도 보지 않는다. 그 침묵을 건수와 함께 출력 | 실측: 킷은 구조화 관계 100건을 쓰는데 소비 프로젝트 두 곳은 **0건**이고 자유참조만 쓴다(finops 11건·PM 다수). 즉 FR-002가 그 레포들에서 한 번도 돌지 않았는데 신호가 없었다 — 'hard 선언 + 무판정'과 같은 계열의 거짓 안전 |
| 2026-07-28 | **관계 대상 이름 정규화 결함 수정** — `resolveRelations` 호출부가 `parseRelationEntry`의 원문 이름으로 소유자 색인을 조회했다. 색인은 `normalizeKey`로 채워지므로 대소문자만 달라도 조회가 실패해 hard `missing-target` **오차단**이 났다. 게이트 e2e 회귀 테스트 3건 추가 | 실측(소비 프로젝트 finops): 스펙이 소유 키 `IacActionRun`을 **글자 그대로 베껴 써도** 차단됐고, 소문자로 바꿔야 통과했다. 즉 FR-002가 요구하는 "소유 Entities 키와 일치"를 만족하는 스펙이 막히는 상태였다. **킷 자기적용으로는 영구히 안 보였다** — 킷의 entity 키(`retag`·`key-anchor`)가 이미 소문자라 정규화 전후가 같다. 결함이 순수 코어가 아니라 **호출부**에 있어 기존 단위 테스트도 못 잡았다 → 게이트 e2e로 고정했다(수정을 되돌리면 2건 실패하는 것 확인) |
| 2026-08-10 | 관계 표기 픽스처의 복사 목록을 폐포 계산으로 교체 | 같은 드리프트 결함 |
