...
| 2026-08-02 | 킷 자기적용 미결 후보 3건 착지 — 실재 판정 3분산(026 entity·029 surface·030 engine/event)과 저장소 밖 진실 2분산(022 배포 스키마·032 라이브 실물)을 **비중복**으로 기각 원장에 사유와 함께 기록. R10 clean | 두 묶음 다 "같은 축, 다른 역할·SSOT"다 — 실재의 근거가 역할마다 다르므로 스펙이 갈리는 것이 분산이 아니라 역할 경계다. 022↔032는 향후 022를 032의 한 kind로 흡수할 여지를 함께 적어 뒀다(그때 migrate-on-deploy 의미가 일반 diff에 묻히는 것이 대가). 작성=LLM·확정=사람이므로 원장 한 줄 삭제로 되돌아온다 |
