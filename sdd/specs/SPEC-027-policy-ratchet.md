...
- **FR-010** (event): WHEN the policy is advisory or hard and the base config is available, the **policy-ratchet** (E) judgment in **policy-ratchet-lib.mjs** (S) SHALL additionally classify a knob whose enforcement is defined by scope rather than by strength or by threshold — a registration/allow-list whose entry count shrank, an exclusion/retirement list whose entry count grew, or a pointer knob (`specDir`, `smokeManifest`, `derivationManifest`, `specSyncBase`, `commands.test`) whose value changed at all relative to base — as a violation, excluding knobs named in `policyRatchetExceptions`, so that `entityRegistry:{}`, `ignoreDirs+="src"`, and a silent `specDir`/`commands.test` redirect cannot defeat every other ratchet while `policyRatchetPolicy` stays hard.
...
- **structural knob** — a config knob whose enforcement is defined by scope, not by an off/advisory/hard rank or a numeric threshold: an allow-list (shrinking it removes protection), an exclusion list (growing it removes coverage), or a pointer that names *where* enforcement looks (changing it redirects enforcement itself). `capabilityVerbs` is deliberately excluded — its growth is ordinary verb registration, indistinguishable from abuse without the reason-per-entry design reserved for a separate change.
...
- **SC-004**: 이슈 #21 A-3 재현 픽스처(`policyRatchetPolicy: hard` 유지, `entityRegistry:{}`·`ignoreDirs+="src"`·`commands.test:"true"`·`specDir` 재지정·`relationTypes`/`strictSpecs` 축소·`retiredIds` 확장)에서 전부 위반으로 지목되고 exit 1, 예외 선언 시 부채로만 표면화되며 exit 0(양판 바이트 동일). [검증: tooling/__tests__/policy-ratchet.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs]
