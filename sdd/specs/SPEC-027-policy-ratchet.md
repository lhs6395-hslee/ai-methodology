...
- **FR-010** (event): WHEN the policy is advisory or hard and the base config is available, the **policy-ratchet** (E) judgment in **policy-ratchet-lib.mjs** (S) SHALL additionally classify a knob whose enforcement is defined by scope rather than by strength or by threshold — a registration/allow-list whose entry count shrank, an exclusion/retirement list whose entry count grew, a pointer knob (`specDir`, `smokeManifest`, `derivationManifest`, `specSyncBase`, `commands.test`) whose value changed at all relative to base, or a boolean requirement switch (`requireAccounting`) that went from true to false — as a violation, excluding knobs named in `policyRatchetExceptions`, so that `entityRegistry:{}`, `entitySchemaSources:[]`, `ignoreDirs+="src"`, a silent `specDir`/`commands.test` redirect, and `requireAccounting:false` cannot defeat every other ratchet while `policyRatchetPolicy` stays hard.
...
- **structural knob** — a config knob whose enforcement is defined by scope, not by an off/advisory/hard rank or a numeric threshold: an allow-list (shrinking it removes protection), an exclusion list (growing it removes coverage), a pointer that names *where* enforcement looks (changing it redirects enforcement itself), or a boolean requirement switch (turning it off removes the requirement outright). `capabilityVerbs` is deliberately excluded — its growth is ordinary verb registration, indistinguishable from abuse without the reason-per-entry design reserved for a separate change. A knob named `*Exempt*`/`*Exception*` is also excluded from this hand-curated set — `classifyExemptionRatchet`'s name-convention auto-detection already watches every such knob generically, so hand-listing one here would duplicate that coverage under a second, easier-to-forget mechanism.
...
- **SC-004**: 이슈 #21 A-3 재현 픽스처(`policyRatchetPolicy: hard` 유지, `entityRegistry:{}`·`entitySchemaSources:[]`·`ignoreDirs+="src"`·`commands.test:"true"`·`specDir` 재지정·`relationTypes`/`strictSpecs` 축소·`retiredIds` 확장·`requireAccounting:false`)에서 전부 위반으로 지목되고 exit 1, 예외 선언 시 부채로만 표면화되며 exit 0(양판 바이트 동일). [검증: tooling/__tests__/policy-ratchet.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs]
...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-27 | 래칫 감시 목록에 `capabilityVerbPolicy` 추가(Node·Python 동시) | SPEC-002 FR-011 동반 — 새 강도 knob이 래칫 밖에서 태어나면 hard→advisory 한 줄로 미등록 verb hard-block을 회피할 수 있다(이슈 #21 E-5가 고친 결함을 다시 여는 경로) [검증: tooling/__tests__/policy-ratchet.test.mjs] |
| 2026-08-27 | FR-010 보강 — `entitySchemaSources`를 `RATCHETED_SETS_SHRINK`에 추가(비면 SPEC-026 백킹 hard가 무음 inert) + 네 번째 모양 **불리언 요구 스위치**(`RATCHETED_BOOLEANS = ["requireAccounting"]`, true→false만 완화) 신설. `entitySchemaExemptEntities`·`prefixClassExemptions`는 이 손 목록에 추가하지 않음 — `Exempt`/`Exception` 이름 규약을 가진 knob은 `classifyExemptionRatchet`이 이미 자동 감시하므로 손 목록 중복 등재는 새 사각지대(두 목록 중 하나만 갱신하고 잊는 경로)를 만든다. Node·Python 패리티 | 이슈 #21 A-3의 21항목 재점검(row 6 "래칫 축 일반화" 후속): 21개 중 위 FR-010 초안이 이미 닫은 항목·이름규약 자동 감시가 이미 닫힌 항목·의도적 설계 제외(capabilityVerbs) 항목을 빼면 `entitySchemaSources`·`requireAccounting` 둘만 실제 사각지대로 남아 있었다 [검증: tooling/__tests__/policy-ratchet.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs] |
| 2026-08-27 | FR-010 신설 — **구조적 knob 래칫**: 강도(off/advisory/hard)도 상한(RATCHETED_LIMITS)도 면제 개수(FR-009)도 아닌 네 번째 축. `RATCHETED_SETS_SHRINK`(`entityRegistry`·`relationTypes`·`strictSpecs`·`testFileRegex` — 원소가 줄면 위반), `RATCHETED_SETS_GROW`(`ignoreDirs`·`retiredIds` — 원소가 늘면 위반), `RATCHETED_POINTERS`(`specDir`·`smokeManifest`·`derivationManifest`·`specSyncBase`·`commands.test` — base 대비 값이 바뀌면 위반, 방향 무관). 셋 다 기존과 같은 `policyRatchetExceptions` 출구를 재사용(부채로 표면화). `capabilityVerbs`는 의도적으로 제외(이슈 #21 E-5/E-6 별도 과제 — 성장이 정상 등록과 구분 안 되어 사유-per-entry 설계가 따로 필요). Node·Python 패리티 | 감사 이슈 #21 A-3 실측: `policyRatchetPolicy: hard`를 유지한 채로 `entityRegistry:{}`·`ignoreDirs+="src"`·`specDir` 재지정·`commands.test:"true"` 등 21개 knob을 한 커밋에 적용해도 기존 세 래칫(강도·상한·면제개수)은 `violations:0 · exit 0`이었다 — "감시·강제의 범위 자체"를 보는 코드가 없었다. `entityRegistry:{}` 재현: 등록 요구가 전체 비활성화돼도 base 대비 판정이 없었다(이제 `entityRegistry: 1 → 0` hard exit 1) [검증: tooling/__tests__/policy-ratchet.test.mjs, tooling/__tests__/sdd-gates-py.test.mjs] |
| 2026-08-26 | 래칫 감시 목록에 `deployWindowPolicy` 추가(Node·Python 동시) | SPEC-060 동반. 새 강제 knob이 래칫 밖에서 태어나면 조용히 하향될 수 있고, 이 knob의 하향은 곧 "배포 시간창을 안 지켜도 됨"으로 되돌아간다 [검증: tooling/__tests__/policy-ratchet.test.mjs] |
| 2026-08-10 | 래칫 감시 목록에 `diagnosisGuardPolicy` 추가 | SPEC-053 동반. 새 강제 knob이 래칫 밖에서 태어나면 조용히 하향될 수 있고, 이 knob의 하향은 곧 "조사 전에 명세를 안 봐도 됨"으로 되돌아간다 [검증: tooling/__tests__/policy-ratchet.test.mjs] |
| 2026-08-10 | 래칫 감시 목록에 `specConflictPolicy` 추가 + 수치 전수성 테스트가 **주석을 판정 대상에서 제외**하도록 교정(킷의 `stripFullLineComments` 재사용) | SPEC-052 동반. 후자는 실측 오탐이다 — 새 knob을 설명하는 주석에 내부 opts 키(`maxDocFreq`)를 적었더니 이 테스트가 그것을 config 임계로 오인해 "래칫 밖"이라 보고했다. **주석 속 이름은 인용이지 선언이 아니다**(SPEC-044 계열). 테스트를 우회하지 않고 스캔 범위를 코드로 좁혔다 [검증: tooling/__tests__/policy-ratchet.test.mjs] |
...
| 2026-08-10 | `preEditSpecFirstPolicy`를 래칫 감시에 편입 + 래칫 전수성 테스트의 열거 집합을 **킷 config → DEFAULTS**로 교체(양판) | 이전 판은 **킷이 선언한** knob만 열거했다. 측정: 강도 enum knob 40종 중 **18종이 킷 config 미선언**이라 사정거리 밖이었고, 그 사각에서 `preEditSpecFirstPolicy` 1종이 래칫 감시 없이 살아 있었다 — 소비 프로젝트가 그 knob을 켜면 hard→advisory 한 줄로 회피할 수 있었다. **열거기가 잘못된 집합을 보면 그 전수성은 전수성이 아니다** |
| 2026-08-11 | 래칫 목록에 `frPlacementPolicy`(SPEC-056)·`gateFailureEscalationPolicy`(SPEC-057) 추가(양판) + Node↔Python 목록을 **직접 대조**하는 패리티 테스트 신설(`runtime-contract.test.mjs` ⑥) | 새 knob 2종 도입과 함께 등재. 대조 테스트를 짜는 과정에서 그 테스트 자신의 정규식이 숫자 포함 knob(`e2eTestsPolicy`)을 놓치는 결함을 발견해 같은 커밋에서 교정했다 — 손으로 두 목록을 나란히 유지하는 동안 한쪽은 반드시 뒤처지고, 대조 장치 자신도 전수성을 잃을 수 있다는 실측이다 |
