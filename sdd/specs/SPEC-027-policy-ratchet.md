...
| 날짜 | 변경 | 근거 |
|---|---|---|
| 2026-08-17 | `RATCHETED_POLICIES`(Node·Python 양판)에 `duplicateSourceDriftPolicy`·`invariantGuardPolicy`(SPEC-059) 추가 | 신규 정책 knob이 래칫 목록에서 빠지면 hard→advisory→off로 조용히 하향돼도 아무도 잡지 못한다(이 목록 자체가 그 결함을 막으려고 만들어졌다 — 새 정책을 만들 때마다 반드시 함께 등재해야 한다) |
