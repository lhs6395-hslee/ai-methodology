...
`--gate=node`로 init하면 `scripts/sdd-sync.mjs`·`/sdd-sync` 스킬·`scripts/sdd-pre-push.sh`가 설치된다(계약: 키트 `HARNESS.md`). spec/코드 변경 후 또는 수시로 `/sdd-sync`로 R1~R10(spec→code·code→spec·dedup+입도·상시 sync)를 사람 확인 게이트로 정렬한다. push마다 점검하려면: `ln -sf ../../scripts/sdd-pre-push.sh .git/hooks/pre-push`(기본 비차단, `SDD_SYNC_BLOCK=1`로 차단).
...
동기화: /sdd-sync (R1~R10 집계·drift 점검), pre-push 훅
```
...
## §13b. 라이브 우선 대조 — "어느 쪽이 정본인가"(회귀 방향 규범)

> **적용 전 라이브와 대조하라. 라이브가 저장소보다 최신이면 저장소를 먼저 라이브에 맞춘 뒤(drift 흡수) 변경을 얹는다.** 낡은 저장소 파일을 정본으로 믿고 그대로 apply하면 **라이브가 되돌아간다**.

실측 사고(소비 프로젝트 gsn-ai-pm): 라이브 ConfigMap이 저장소보다 최신이었다(라이브는 마이그레이션된 `statistic` 단수, 저장소는 deprecated `statistics` 배열). 저장소를 정본으로 믿고 apply한 결과 **정상 렌더되던 패널까지 전멸**했다. 같은 취지 문장이 그 프로젝트 INFRA-002 Change Log에 이미 있었으나 개별 기록이라 다른 스펙으로 전이되지 않았다 — 그래서 방법론 차원 규범으로 승격한다.

**절차(인프라·매니페스트 변경 전 필수):**
1. **대조 먼저** — 라이브 오브젝트/state를 조회해 저장소와 내용 해시를 비교한다(`check-live-reality`의 kubernetes·terraform 검사가 이 대조를 기계화한다, SPEC-032).
2. **방향 판정** — 어긋나면 *어느 쪽이 최신인가*를 먼저 답한다. 저장소가 최신이면 정상 apply. **라이브가 최신이면 흡수 먼저**: 라이브 상태를 저장소로 역반영(export·import)해 커밋한 뒤, 그 위에 새 변경을 얹는다.
3. **out-of-band 자원은 소유자를 만든다** — IaC state에 없는 실물(CLI로 만든 자원)은 `import`로 state에 편입하거나 명시적으로 폐기한다. 무소유 자원은 사라져도 아무도 복구하지 않는다(실측: NAT Gateway가 state에도 스케줄에도 없어 egress 상실 위험).
4. **기록** — 무엇이 어긋났고 어느 방향으로 해소했는지 해당 인프라 스펙 **Change Log에 남긴다**(다음 사람이 같은 방향 판정을 다시 하지 않도록).

**금지:** "저장소가 정본이니까"를 근거로 대조 없이 apply하는 것. 정본은 *선언*이지만 **진실은 실물**이고, 둘이 갈라진 상태에서 선언을 밀어붙이면 진실이 파괴된다.
