...
- **실재·증거·의미 축 확장 7차(2026-07-21~08-02)** — "선언이 실제로 동작하는가"를 새 축으로 세우고, 소유권 분류 감사(41건)의 지적을 봉합했다.
  - **소유권 분류 재정초(SPEC-001 FR-010·SPEC-030)**: 판정을 카테고리 *이름*이 아니라 **역할**(`ownershipCategoryRoles`)에 걸어 개명 한 줄로 강제가 죽던 구멍을 닫고, 칸이 없어 우회하던 순수 로직·비동기 신호에 **Engines·Events** 역할을 신설
  - **실재 판정 3축(SPEC-026·029·030)**: entity=구조 SSOT · surface=심볼 정방향 · engine/event=모듈·카탈로그. orphan(역방향)과 짝을 이뤄 양방향이 닫힘
  - **소유 구멍 3종(G1·G2·G3)**: 소유 미선언 스펙 · 카테고리 교차 중복 · Files 글롭 겹침
  - **정책 래칫(SPEC-027)**: 강도 하향 차단, **자기포함** — 이 knob을 내려 판정을 끄는 자폭 불가
  - **판정 회계 맵(SPEC-028)**: 소유 키마다 어느 가드가 실제로 봤는지·**미판정**인지 생성물로 가시화
  - **실행 증거·라이브 대조(SPEC-031·032, R8·R9)**: `[검증]` 태그를 실행 가능한 경로로 강제 + 저장소 밖 실물과의 diff(자격증명 없으면 `skipped(사유)`)
  - **의미적 중복 3층(SPEC-033, R10)**: 형태 변이·선언 동의어는 결정적 차단, 유사 후보는 **어떤 강도에서도 비차단** + 후보 목록 신선도. 생성기는 **세션 LLM**(561쌍 실측 근거, 아래 보류 항목 해소)
  - **스펙 마이그레이션(SPEC-025)·Capability 귀속(SPEC-024)**: update가 표면화한 백로그를 승인 경유 실제 재구성
...
## 📋 진행 중 백로그 (advisory로 표면화 — hard 승격 전 소진 대상)
| 항목 | 규모 | 승격 조건 |
|---|---|---|
| **SC·NFR 검증 바인딩**(SPEC-034, `scCoveragePolicy: advisory`) | 킷 자신 미회계 **99/110** | 각 SC에 `[검증: 경로]`를 달거나 `evidenceManifest`로 증거·유예 회계 → 0건이 되면 `hard`. 래칫이 감시하므로 되돌릴 수 없다 |
| **FR 키 앵커**(`frKeyAnchorPolicy: advisory`) | — | 앵커 정리 후 `hard` |
...
| ~~하네스: R3 임베딩 의미리뷰~~ → **2026-08-02 해소·미채택 확정**(SPEC-033) | 킷 entity 34건 **561쌍 전수 판정** 실측: 값싼 어휘 툴은 이름 축 7/7을 상위 20에 올렸지만 **기능 축은 0/11**(중앙 84위, 최악 353위). 이름 축은 ①이 이미 보는 영역이라 외부 임베딩(Ollama·HuggingFace·model2vec)의 순증이 없어 **제외**. 후보 생성은 세션 LLM이 맡고 전수성은 열거기가 보장한다 |
...
| ~~**Surface 실재 검증 축**~~ → **2026-07-28 착수·완료**(SPEC-029 심볼 문법). 보류 사유였던 "어댑터를 어디서 얻나"를 **문법으로 우회**했다 — 라우트 매니페스트·OpenAPI 같은 외부 신호를 주입받는 대신, 파일형 surface 키는 선언된 `ownershipSourceRoots` 아래 실재하는 파일·디렉토리여야 한다는 규칙으로 판정한다. 어댑터가 필요 없고 우회도 불가능하다. 남은 부분: **HTTP 라우트**(`POST /api/x`)의 실재는 여전히 축이 없다 — 파일이 아니라 대상에서 자동 제외되며, 필요 증명 시 라우트 매니페스트 어댑터를 별도로 신설한다 |
