# 스펙 간 중복 — 3계층 차단 (결정 기록)

> 방법론 최대 빈칸은 "각 spec이 서로 중복인지 판정하는 규칙"이 없다는 것이었다. 키트는 이를 **사람/LLM 판단**에 맡겨 두고 있었다(누락 위험). 이를 **3계층**(구조 유일성 게이트 + 형태·선언 동의어 게이트 + 유사 후보 표면화·좁힌 리뷰)으로 메운다. 시각 설계 동반물(역할 기반 5분류·애니메이션): [`docs/ownership-key.html`](docs/ownership-key.html).
...
- **Capability**: `<entity>.<verb>` — 점 정확히 1개, 소문자, verb ∈ CRUD 기본(`create·read·update·delete·list`) + config `capabilityVerbs` 등록 verb만. `capabilityVerbs`는 배열(레거시)뿐 아니라 `{동사:사유}` 객체도 받는다(entityRegistry와 동형 — 빈 사유는 항상 에러). 미등록 verb = 형식 위반. 임의 동의어 금지. **강도:** 점 개수·Surface 형식 등 다른 형식 위반은 `validateKey`가 잡되 **기본은 ⚠ warn·exit 0**이고 `check-ownership --strict`에서만 exit 1이다(설치 훅·CI 기본 호출엔 `--strict`가 없다, `tooling/harness/pre-commit`). **미등록 verb만은 예외** — 전용 knob `capabilityVerbPolicy`(off\|advisory\|**hard**)가 있어 `--strict` 없이도 hard에서 exit 1한다. **권장 종착지는 `capabilityVerbPolicy: hard`.**
