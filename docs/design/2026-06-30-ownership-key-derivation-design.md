...
현재 `check-fr-coverage`는 등록 안 된 접두어 파일을 **조용히 `continue`(건너뜀)** 한다 — `FEAT-001.md`가 있어도 config에 `FEAT`가 없으면 그 spec의 FR이 통째로 누락된 채 거짓 green(소비 프로젝트 A 실측 사례). 이를 뒤집는다:
...
- 소비 프로젝트 A FEAT → `SPEC`으로 정리하거나 사유 기재해 정식 편입(그쪽 팀 결정).
