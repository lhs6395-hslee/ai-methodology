...
| **Capability 귀속(SPEC-024)** — `entity.verb`의 entity를 그 스펙이 소유(또는 참조 선언)해야(`capabilityOwnershipPolicy`) | `[검증]` **동작·차단(설계대로)**(2026-07-17, 킷 self-test·Node↔Python 패리티) | entity 없이 capability 소유(기술 계층 스펙)·유령 entity(스키마 아님)·남의 aggregate 위 capability = 위반(hard exit 1). 소비 스펙 재현 픽스처(wizard: `pjt_projects.*`·`wizard.*` 미소유)에서 6건 지목 실증. 비-웹 카테고리(킷 Modules 등)·capability 카테고리 부재 시 no-op |
