...
| **실재·증거·의미 게이트(SPEC-023~033)** — 키트 자체 스위트로 검증 | `[검증]` **동작** | 2026-08-02: `node --test tooling/__tests__/*.test.mjs` **532/532 pass**, `sdd-sync` R1~R10 판정(R10만 미결 후보 3건 재부상, exit 0), Node↔Python 바이트 동일(동의어 게이트 3분기 포함), 킷 자기적용 `synonymPolicy: hard`에서 entity 34건 형태 충돌 0 |
