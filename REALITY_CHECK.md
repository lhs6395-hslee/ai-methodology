...
| **실재·증거·의미·회계 게이트(SPEC-023~036)** — 키트 자체 스위트로 검증 | `[검증]` **동작** | 2026-08-02: `node --test tooling/__tests__/*.test.mjs` **561/561 pass**, `sdd-sync` **R1~R12 전부 clean**(킷 자기적용 첫 전면 green — SC·NFR 120건 회계 완료로 R11 백로그 101→0, `scCoveragePolicy: hard`), Node↔Python 바이트 동일, 킷 자기적용 `synonymPolicy: hard`에서 entity 37건 형태 충돌 0. 같은 라운드에 소비 프로젝트 실측 제보 6건 착지 — 마커 오탐·단일 대시 미인식·분류 접미 누락(수정), 정책 교착·advisory≡hard·본문↔매니페스트 모순(설계 판정) |
| **반사성·가장자리 감사 봉합(6차)** — config 자기보호(HEAD-config 판정)·상태 화이트리스트·트레일러/동인 스코프·retire inbound·retiredIds 재사용/001-면제·훅 이식성(pre-merge-commit) | `[검증]` **동작**(2026-07-16, 킷 self-test 327/327·Node↔Python 패리티) | config 약화+코드 변경 한 커밋=HEAD config로 exit 1, Planned/enum 밖 소유 코드=exit 1, 트레일러+unowned(error)/글롭 위반=exit 1(우회 불가), `Change-Driver @glob` 스코프 밖 파일=위반 유지, 폐기 ID 재사용=hard, `sdd-retire SPEC-011` dry-run이 inbound 참조 4건 지목(킷 자신 실증). **킷 자체 CI(CICD-001)도 `[검증]`** — PR #1 첫 실행 green(gates job, 2026-07-17, 19초 — build-evidence 확보) |
| **FR 키 앵커(SPEC-023)** — FR 라인 평문 bold ↔ 소유·참조 키 대조(`frKeyAnchorPolicy`) | `[검증]` **동작**(2026-07-17, 킷 self-test·Node↔Python 바이트 패리티) | off=출력 무변(하위호환), advisory=미매치 ⚠·exit 0, hard=미매치 exit 1, 코드 스팬(`` `...` ``)·FR-ID 제외. 킷 advisory 상시 on — 도입 즉시 수사적 bold 1건(SPEC-003 "beginning") 실수확·정리 |
| **Capability 귀속(SPEC-024)** — `entity.verb`의 entity를 그 스펙이 **소유**해야(`capabilityOwnershipPolicy`, 기본 `advisory`) | `[검증]` **동작·차단(설계대로)**(2026-07-17, 킷 self-test·Node↔Python 패리티) | entity 없이 capability 소유(기술 계층 스펙)·남의 aggregate 위 capability = 위반(advisory ⚠·hard exit 1). **참조 선언(`## Dependencies`)은 귀속 근거가 아니다** — 판정은 소유 Entities 집합만 본다(`capability-ownership-lib.mjs`가 `ownedEntities`만 입력받음), 참조 entity 위의 capability도 위반(SPEC-024 Edge Case). 해소는 (a) 그 entity 소유 스펙으로 이관(verb가 달라도 같은 스펙에 FR 신설) 또는 (b) 이 스펙이 그 aggregate면 Entities에 소유 선언. 소비 스펙 재현 픽스처(wizard: `pjt_projects.*`·`wizard.*` 미소유)에서 6건 지목 실증. 비-웹 카테고리(킷 Modules 등)·capability 카테고리 부재 시 no-op |
| **Entity 스키마 백킹(SPEC-026)** — 소유 entity가 구조 SSOT에 실재(유령 entity 차단, `entitySchemaBackingPolicy`) | `[검증]` **동작**(2026-07-21, 킷 self-test·Node↔Python 패리티) — 단 **기본값 `off`** | **유령 entity(스키마 아님)는 SPEC-024가 아니라 이 게이트 소관**이고, 기본 설정에선 **무판정**이다: `off`(기본) + `entitySchemaSources` 비어있음 + entity류 카테고리 부재, 이 셋 중 하나라도면 inert. 켜면 advisory ⚠·hard exit 1이고 면제(`entitySchemaExemptEntities`)는 매 실행 부채로 표면화(FR-005). **2026-07-28 갱신 — 킷 자신도 이제 `hard`로 발화한다**(종전엔 `Modules` 카테고리라 inert였다): 역할을 `ownershipCategoryRoles`로 선언하고 모듈 문법(SPEC-029 ①, `{kind:"spec-slug"}`)을 소스로 채택해 29/29 실재·면제 0 |
| **소유 키 실재 문법(SPEC-029)** — 모듈 문법(entity 키 = 스펙 파일명 슬러그)·심볼 문법(surface 키 = 소스 루트 아래 실재 파일·디렉토리) | `[검증]` **동작**(2026-07-28, 킷 self-test 29/29·52/52·**예외 0**, Node↔Python stdout·stderr·exit 동일) — 기본값 둘 다 `off` | 어댑터(정규식) 위임의 실측 우회 4경로와 **surface 정방향 판정 부재**(감사 M-2)를 문법으로 닫는다. 모듈 문법은 **스펙별** 대조라 슬러그 뒤바뀜도 잡는다(전역 집합이면 못 잡음 — 회귀 테스트 고정). 파일형 아닌 표면(`POST /api/x`·`event:`·`job:`·`/path`)은 자동 제외 → HTTP 표면 레포 오발동 0. `hard`인데 소스 루트 미선언이면 거짓 안전이라 exit 1. **예외 목록 없음** — 불일치는 데이터 교정으로 닫는다 |
| **Ownership 보증 맵(SPEC-028)** — 키별 유일성·앵커·실재를 `✓`/`✗`/`면제`/**`미판정`**으로 | `[검증]` **동작**(2026-07-28 킷: 소유 키 129·칸 516·**미판정 0**) | 게이트는 위반만 출력하므로 초록 화면이 "검증됨"과 "아무도 안 봤음"을 구분 못 한다 — `미판정` 칸이 그 구분을 담당한다. 도입 시점 킷은 미판정 79칸·가드 2종이었고, SPEC-029 + Capabilities 카테고리 도입으로 **0**이 됐다. **드리프트 강제점 배선됨**(2026-07-30) — `sdd-sync` R3가 `gen-ownership-map --check`(읽기 전용)를 스윕에 포함해 맵이 낡으면 규칙이 flagged된다. pre-push·CI가 sdd-sync를 타므로 사람이 재생성을 기억할 필요가 없다(실측: SPEC-031·032 추가 직후 드리프트를 이 배선이 잡음) |
...
| **Spec-ID 번호 무결성(SPEC-014)** — 접두어별 001·중복·중간 gap | `[검증]` **동작**(2026-07-06) | 접두어별 `(prefix,num)` 중복·최소번호≠001 exit 1(hard), 001..max 중간 gap advisory(`--strict` hard). numbering-lib→check-fr-coverage 배선. Node↔Python 바이트 패리티 green(셸/Go 미포팅) |
| **TEST 삭제가능 도메인(SPEC-015)** — `testInfraGlobs` 격리·`Lifecycle: removable` | `[검증]` **동작**(2026-07-06) | `testInfraGlobs` 매치 인프라 파일을 비-TEST 스펙이 소유하면 exit 1(제품 누수 차단), TEST 스펙은 prefix-class 인프라 요구 면제, 기본 `[]`=비활성(하위호환). check-fr-coverage 배선. Node↔Python 패리티 green(셸/Go 미포팅) |
| **오브젝트 스토리지 결정(SPEC-016)** — `objectStorageMarkers`·`## Object Storage Decision` | `[검증]` **동작**(2026-07-06) | 마커 매치 스펙에 Bucket·Consolidation 라벨 섹션 없으면 completeness advisory(`--strict` exit 1), 기본 `[]`=비활성. check-spec-completeness 배선. Node↔Python 패리티 green(셸/Go 미포팅) |
| **Entity 관계(SPEC-017)** — `Entity (relation-type)` 실재·순환 | `[검증]` **동작**(2026-07-09) | Dependencies 구조화 관계의 대상 Entity 미실재=exit 1(hard), aggregate 간 순환=advisory, `relationTypes` 등록 시 미등록 type=exit 1(빈 목록=형식만). 괄호 없는 항목은 레거시 통과(opt-in). check-ownership 배선. Node↔Python 패리티 green(셸/Go 미포팅) |
| **환경 tier 검증** — `commands.test`(로컬 안전)/`commands.smoke`(인프라)·능동적 probe | `[조건부]` **배선·규율**(게이트 강제 아님) | `sdd-run`이 `smoke` stage를 기계 실행하나, tier 분리·probe 판정·skip 가드는 프로젝트가 배선하는 규율(METHODOLOGY "검증은 환경으로 계층화"·`presets` 테스트 tier·`ci-examples`). CSP 무관 — 게이트가 강제하지 않음을 명시 |
