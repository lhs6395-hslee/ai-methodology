...
| **실재·증거·의미·회계 게이트(SPEC-023~036)** — 키트 자체 스위트로 검증 | `[검증]` **동작** | 2026-08-02: `node --test tooling/__tests__/*.test.mjs` **561/561 pass**, `sdd-sync` R1~R12 판정(R11 SC·NFR 회계만 advisory 백로그, 나머지 clean), Node↔Python 바이트 동일, 킷 자기적용 `synonymPolicy: hard`에서 entity 37건 형태 충돌 0. 같은 라운드에 소비 프로젝트 실측 제보 6건 착지 — 마커 오탐·단일 대시 미인식·분류 접미 누락(수정), 정책 교착·advisory≡hard·본문↔매니페스트 모순(설계 판정) |
