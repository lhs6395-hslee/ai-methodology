...
| **Ownership 보증 맵(SPEC-028)** — 키별 유일성·앵커·실재를 `✓`/`✗`/`면제`/**`미판정`**으로 | `[검증]` **동작**(2026-07-28 킷: 소유 키 129·칸 516·**미판정 0**) | 게이트는 위반만 출력하므로 초록 화면이 "검증됨"과 "아무도 안 봤음"을 구분 못 한다 — `미판정` 칸이 그 구분을 담당한다. 도입 시점 킷은 미판정 79칸·가드 2종이었고, SPEC-029 + Capabilities 카테고리 도입으로 **0**이 됐다. ⚠ **드리프트 강제점 없음** — `--check`가 훅·CI에 배선되지 않아 사람이 재생성을 기억해야 한다 |
