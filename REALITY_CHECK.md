...
| **Entity 스키마 백킹(SPEC-026)** — 소유 entity가 구조 SSOT에 실재(유령 entity 차단, `entitySchemaBackingPolicy`) | `[검증]` **동작**(2026-07-21, 킷 self-test·Node↔Python 패리티) — 단 **기본값 `off`** | **유령 entity(스키마 아님)는 SPEC-024가 아니라 이 게이트 소관**이고, 기본 설정에선 **무판정**이다: `off`(기본) + `entitySchemaSources` 비어있음 + entity류 카테고리 부재, 이 셋 중 하나라도면 inert. 켜면 advisory ⚠·hard exit 1이고 면제(`entitySchemaExemptEntities`)는 매 실행 부채로 표면화(FR-005). 킷 자신은 `Modules` 카테고리라 inert(정직한 델타) |
