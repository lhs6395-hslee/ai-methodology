...
<!-- retrofit(기존 코드에 사후 스펙): Draft로 두지 말고 Reviewed로 바로 작성. 이미 있는 코드를 스펙화하는 것은
  작성 시점에 코드 대조 검토가 끝난 상태라, Draft면 spec-sync가 소유 코드(이미 존재) 동반 변경을 차단해
  매 스펙 Draft→커밋실패→Reviewed 왕복이 생긴다. Review Log 한 줄 채우고 Reviewed로. Draft는 코드가 아직 없는 신규 기능만. -->
...
<!-- 이 절은 **현재도 유효한 제약**이 사는 자리다. 본문(FR·SC·Ownership)은 "현재 진실"만 담고
     지나간 상태는 지운다(STRUCTURE §폐기). 그럼 "왜 그렇게 안 했나"가 사라지는데, 그 기억이
     없으면 같은 제안이 몇 달 뒤 다시 올라온다. 그래서 세 자리로 나눈다:
       본문 = 지금 무엇인가 / Change Log = 언제 무엇을 왜 바꿨나(사건) / 여기 = 왜 그 대안이 아닌가(제약)
     Change Log는 시간이 지나면 아래로 밀려 안 읽힌다 — 상시 유효한 판단은 여기 남긴다. -->
- **기각한 대안:** [무엇을 왜 안 했나 — 재검토 조건이 있으면 함께. 예: "RDS 서버리스 v2는 ACU
  스파이크 비용과 콜드스타트 p95 초과로 기각(2026-08). 재검토 조건: 워크로드가 야간 유휴 8h↑로
  바뀌면"] — 없으면 `—`
