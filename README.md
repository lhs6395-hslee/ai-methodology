...
> - **정책 래칫**: 강제 강도 하향 차단(자기포함) + **수치 임계 상향 차단**(`maxFRsPerSpec` 등 — 캡 초과는 분할·병합으로 해소하는 것이지 자를 늘려 재는 것이 아니다) — advisory는 경유지, hard는 종착지, SPEC-027
...
> **문서를 어디에 두나 — 루트냐 `docs/`냐(규범).** 갈림은 "설명이냐 기록이냐"가 아니라 **소비 프로젝트가 그 파일을 읽어야 하는가**다.
> - **루트 `*.md` = 방법론 SSOT.** 규율 자체를 정의하고 에이전트가 채택 시 raw로 직접 읽는다(`METHODOLOGY`·`STRUCTURE`·`STORAGE`·`HARNESS`·`APPLYING`·`principles`…). 경량 부트스트랩의 sparse checkout이 **cone 모드로 루트 파일 전부**를 받고 `docs/`는 제외하므로(§경량 부트스트랩), 규범 문서를 `docs/`로 옮기면 **소비 프로젝트가 그 규범을 못 읽는다** — 위치가 곧 배포 경계다.
> - **`docs/` = 이 레포에서만 사는 산출물.** 시각 설명서(`*.html`)·날짜별 설계 기록(`design/`)·감사 보고서·생성물(`change_log.html`)·예제 코드(`examples/`). 킷을 *쓰는* 데는 필요 없고 킷을 *이해·감사*하는 데 쓴다.
> - 새 문서를 만들 때: 그 문서가 없으면 소비 프로젝트가 규율을 따를 수 없나? → 루트. 아니면 → `docs/`. 애매하면 루트에 두지 말고 물어라(루트는 배포 대상이라 늘어날수록 모든 채택자가 비용을 낸다).
...
| [`HARNESS.md`](HARNESS.md) | **인터랙티브 spec↔code sync 계약**(규칙표 R1~R12·실행기 `/sdd-sync`·pre-push 훅) + **강제 hook 세트**(SessionStart·PreToolUse·pre-commit·pre-push, `sdd-init` 배선) + **spec-first**(`check-spec-sync`·commit-msg hard·`/speckit.fix` — Files glob 소유매핑, changeset=브랜치, Spec-Impact 트레일러) | ★ 하네스 |
