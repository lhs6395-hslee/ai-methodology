...
> - **라이브 대조 등록 축**: 새 배포 산출물을 선언하면 그것을 보는 검사 **등록**을 요구 — 실행 축과 정책이 분리돼 자격증명 없는 로컬에서도 판정된다 — SPEC-032
> - **검증 실행 회계**: 선언한 `[검증]` 자산이 **돌았는지** — 실행 원장과 대조해 "기록 없음(침묵)"을 잡는다. 포기는 허용하되 사유 없는 포기는 기록이 아니다 — SPEC-041
> - **게이트 판정 타입**: 게이트가 무엇을 했는지를 산문이 아니라 타입으로 — `JUDGED`만 초록의 자격을 갖고 `OFF`·`INERT`·`SKIPPED`·`UNTYPED`는 전부 "안 봄"이다. 스윕 요약이 언제나 분모를 밝힌다(`판정 M · 안 봄 K · 미판정 J`) — SPEC-040
...
> **이 표의 배치 규칙**(권장 정본: [`STRUCTURE.md`](STRUCTURE.md) §"프로젝트 설명 문서는 어디에 사나"). 킷에 적용하면 — **루트 `*.md` = 채택자가 읽어야 하는 규율**(경량 부트스트랩 sparse checkout이 루트 파일 전부를 받고 `docs/`는 제외하므로, 규범을 `docs/`에 두면 소비 프로젝트가 못 읽는다), **`docs/` = 이 레포에서만 사는 산출물**(시각 설명서·설계 기록·감사·생성물·예제).
