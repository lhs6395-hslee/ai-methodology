...
2. **PREFIX 표준 4종**: `SPEC`(제품 기능·런타임 동작, 기본) · `INFRA`(프로비저닝 자원 — 네트워크·컴퓨트·DB·스토리지·IAM 등 선언적 클라우드/플랫폼 자원) · `CICD`(CI/CD·릴리스 자동화 — 소스 변경→빌드·테스트·게이트→아티팩트/이미지 게시→배포 트리거의 **동작·정책**; 검증은 build-evidence(파이프라인·배포 로그), 수명주기 permanent) · `TEST`(테스트/QA 도메인 — 테스트 스위트 + **런타임·전용 인프라를 자기완결로 소유하는 삭제 예정 비제품 도구**; `Lifecycle: removable` 관례, TEST 스펙의 인프라 소유는 prefix-class 면제, 테스트 인프라는 `testInfraGlobs` 네임스페이스로 제품과 격리 — SPEC-015). `specIdPrefixes` 기본값 = `["SPEC","INFRA","TEST","CICD"]`. (표준 밖은 `prefixRationale` 등록 필수 — `check-fr-coverage`가 강제)
   ...
   - **조용한 누락 제거**: 게이트는 `specDir`의 **모든** `^[A-Z]+-\d{3}.*\.md` 파일을 스캔한다. 허용 집합 밖 접두어 파일이 있으면 **조용히 건너뛰지 않고 exit 1**. (과거: 미등록 접두어 파일은 조용히 `continue`되어 그 spec의 FR이 추적에서 통째로 빠져 거짓 green — 소비 프로젝트 A FEAT-001 실측 사례.) ← 가장 흔한 함정.
...
4. **추적 닻 = 언어중립 ID** `FR-NNN`. 테스트는 `@covers <PREFIX>-NNN/FR-NNN`(주석 스타일 자유 `//`·`#`·`--`)로 연결.
   - **FR 번호 = 스펙별 001부터 연번**(표준·정본). 완전 식별자는 `<SPEC-ID>/FR-NNN`이고 **앞의 스펙 ID가 이미 네임스페이스**라 뒤 번호는 그 스펙 안에서만 유일하면 된다 — 전역 유일성을 다시 벌 이유가 없고, 그 대가로 가독성을 파는 것은 손해다(전역 연번은 한 스펙의 요구가 `001·047·048·051·066`처럼 갈라진다 — 실측). 그래서 스펙 A의 `FR-001`과 스펙 B의 `FR-001`은 **충돌이 아니다**.
   - **한 스펙 안 번호 중복은 hard(exit 1)**: 같은 요구 번호를 두 번 선언하면 fr 게이트가 `<SPEC-ID>/FR-NNN`을 지목하고 차단한다(정책 knob 없음 — 정당한 케이스가 없다). 실측 사고 계열: 두 브랜치가 같은 스펙에 각자 요구를 추가해 같은 번호가 2회씩 생겼는데 병합이 조용히 통과했다(뒤 선언이 앞을 덮고 `@covers` 한 태그가 두 요구를 만족한 것처럼 회계 → 거짓 green).
   - **001 미시작·중간 결번은 advisory**(`--strict`에서 hard 승격): 결번은 요구 폐기가 남기는 정당한 흔적일 수 있다(`sdd-retire` — SPEC-018). 레터 서픽스(`FR-003a`)는 `FR-003`과 별개 ID이고 기저 번호로 접혀 결번을 만들지 않는다.
   - **스펙 간 요구 이동은 재번호다**: 옮긴 스펙의 다음 번호를 새로 받고, `@covers` 태그·`smokeManifest`는 `sdd-retag`(SPEC-011)로 **기계 이행**한다 — 손 재태깅 금지.
   - 판정 정본: SPEC-014(spec-ID 번호와 같은 aggregate — 판정 원형 공유, severity만 분리). 요구 접두어를 도메인별로 쓰면(`requirementIdPrefixes`: 예 `FR`·`INFRA`) **접두어별로 독립 연번**이다.
