...
**정본 절차(SSOT):** `prompts/adopt.md` 를 **그대로 실행**한다 — 절차 원본은 그 파일 한 곳이다(레포에 저장된 프롬프트를 일관되게 실행). 위치: 키트 `<KIT>/prompts/adopt.md`(기본 `~/Documents/claude/sdd`), 로컬에 없으면 `<methodology-url>` raw(`https://raw.githubusercontent.com/lhs6395-hslee/ai-methodology/main/prompts/adopt.md`). 아래는 그 요약 + 고정 규칙(가드레일) — **원본과 충돌 시 원본 우선**.
...
2. **강제 배선(1회).** 대상 루트에서 `sh <KIT>/tooling/sdd-init.sh --gate=node` → `sdd/` 레이아웃·게이트(~25파일)·git 훅(pre-commit·commit-msg)·SessionStart/PreToolUse·스킬 설치.
