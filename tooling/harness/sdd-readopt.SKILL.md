...
**정본 절차(SSOT):** `prompts/readopt.md` 를 **그대로 실행**한다. 위치: 키트 `<KIT>/prompts/readopt.md`(기본 `~/Documents/claude/sdd`), 로컬에 없으면 `<methodology-url>` raw. 아래는 요약 + 고정 규칙 — **원본과 충돌 시 원본 우선**.
...
3. **강제 재배선.** 대상 루트에서 `sh <KIT>/tooling/sdd-init.sh --gate=node --force` (최신 도구로 덮어씀).
