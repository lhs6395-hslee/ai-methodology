...
# 런타임을 **고른다** — 이전 판은 Node 게이트만 알았고, 그래서 Python 런타임 프로젝트는
# `preEditSpecFirstPolicy=hard`를 켜고도 편집 시점 보호가 0이었다(**hard 선언 + 무판정 = 거짓 안전**).
# 원래 "Python 미러 없음" 결정의 근거도 "미러가 있어도 **호출부가 없다**"였다 — 그 호출부가 여기다.
RUN=""
for cand in scripts/check-pre-edit.mjs tooling/check-pre-edit.mjs; do
  if [ -f "$cand" ] && command -v node >/dev/null 2>&1; then RUN="node $cand"; break; fi
done
if [ -z "$RUN" ]; then
  for cand in scripts/sdd_gates.py tooling/sdd_gates.py; do
    if [ -f "$cand" ] && command -v python3 >/dev/null 2>&1; then RUN="python3 $cand preedit"; break; fi
  done
fi
...
if [ -z "$RUN" ]; then
  echo "[SDD 편집 체크] 검사 못 함 — 판정할 런타임이 없다(node·python3 게이트 모두 부재). 통과가 아니다."
...
if $RUN --is-code-path "$FP" >/dev/null 2>&1; then
  cat <<'EOF'
