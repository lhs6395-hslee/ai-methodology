...
**"spec부터"가 아니라 "재현부터"다** — 에이전트 컨텍스트(`agent-context.md` 진입 규칙 5)의 두 갈래 중 버그 수정 쪽이 이 스킬이다. 재현하지 못한 채 스펙 문구부터 쓰면 원인이 다른 곳이었던 사례가 나온다(찔러보기 금지) — 신규 기능(spec-first)과 순서가 반대인 이유다. 스펙 갱신과 코드 수정이 **같은 커밋**에 들어가면 되고, 어느 쪽을 디스크에 먼저 썼는지는 게이트가 묻지 않는다.
