...
2. **스펙 영향 판정** — 소유 스펙은 `Ownership.Files` glob으로 확인:
   ...
   - **공유 파일을 *다른* 스펙 기능 때문에 고칠 때**(넓은 글롭 `dir/**`로 파일을 포괄 소유한 스펙 A의 파일을 실제로는 스펙 B 기능 때문에 수정) → A에 **잡음 Change Log를 넣지 말고** 커밋 트레일러 `Change-Driver: B <사유>`를 쓴다(SPEC-020). 동인 B가 같은 changeset에서 의미 있게 바뀌었으면 A의 요구가 참조 완화된다. 억지 Change Log는 추적을 왜곡하니 **진짜 동인을 기록**하는 게 기본값이다.
...
## 해소 범위를 물을 때 (확인된 결함이 여러 건일 때)
확인된 결함이 여러 건이면 "어디까지 고칠까"를 묻게 된다. 그때 **부분 수정을 권장으로 올리지 않는다.**
- 기본 권장 = **확인된 것 전부**. 크기가 부담이면 그건 **배치 문제**다 — 한 스펙 = 한 커밋으로 쪼개 순차 착지(빅뱅 금지)하되 범위는 줄이지 않는다.
- "한 라운드에 검증 가능한 크기"는 옳은 이유지만 **순서의 근거이지 범위의 근거가 아니다.** 둘을 섞으면 미수정이 기본값이 된다.
- 진짜 이월이면 **결정으로 기록**한다: 무엇을·왜·언제까지·누가 위험을 수용했나. 넷 중 하나라도 없으면 이월이 아니라 방치다.
- 보안 결함은 특히 — "스펙에만 착지(코드 무변경)"는 취약점을 라이브에 남긴 채 기록만 남기는 것이다. 수용자와 기한 없이는 선택지에 올리지 않는다.
