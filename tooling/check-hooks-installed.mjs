...
// **git은 훅을 복제하지 않는다** — 그래서 갓 체크아웃한 작업본에 훅이 없는 것은 "채택하지 않았다"의
// 증거가 아니다. 이 사실을 무시하면 CI에서 이 게이트는 언제나 위반을 낸다: 스윕을 CI에 배선한 순간
// (그것이 유일한 우회 불가 채널이다 — SPEC-048) `hooksInstalledPolicy=hard` 프로젝트의 빌드가
// **거짓 위반으로** 깨지고, 그 다음에 일어나는 일은 정해져 있다 — 사람이 이 게이트를 끈다.
//
// 완화가 아니다. 관측 불가한 사실을 위반이라 말하지 않는 것이고(SPEC-054의 3분류),
// 강제가 사라지지도 않는다: CI에서 채택의 관측 가능한 대리물은 **채택 영수증**이고 그것은
// R17이 판정한다(영수증에 훅 목록이 실린다). 이 축은 **로컬 채택 축**으로 정직하게 남는다.
const CI_ENV = (cfg.hooksInstalledSkipEnv || ["CI"]).map(String);
const ciEnv = CI_ENV.find((k) => String(process.env[k] ?? "").trim() && process.env[k] !== "false");
if (ciEnv) {
  verdict(VERDICT_KINDS.SKIPPED, `${ciEnv} 환경 — git은 훅을 복제하지 않으므로 갓 체크아웃한 작업본에서 훅 설치는 관측되지 않는다`);
  console.log(`훅 배선 게이트 — ${ciEnv} 환경이라 **판정하지 않았다(통과가 아니다)**.`
