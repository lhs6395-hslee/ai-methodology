...
| `frKeyAnchorPolicy` | FR 키 앵커(SPEC-023) — FR 선언 라인의 평문 bold를 그 스펙의 Ownership∪Dependencies 키와 대조(bold=키 앵커 전용, 코드 스팬은 리터럴). `off`(판정 안 함)·`advisory`(미매치 경고)·`hard`(미매치 exit 1). **권장 종착지 = `hard`**; `off`/`advisory`는 마이그레이션 중 임시 상태 — 기존 수사적 bold를 `/sdd-migrate`로 정리한 뒤 `hard`로 승격한다(update가 백로그 0에서 승격 권장) | `"off"` |
