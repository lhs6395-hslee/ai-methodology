...
| `policyRatchetPolicy` | 정책 강도 **단조 증가** 강제(SPEC-027) — `off`·`advisory`·`hard`. 네 축을 동시에 본다: ① 강도 enum knob(`frKeyAnchorPolicy`·`capabilityOwnershipPolicy`·`entitySchemaBackingPolicy`·`specSyncUnownedPolicy`·`draftBlockPolicy`·`semanticDriftPolicy`·`runTestsPolicy`·`migrationStatePolicy` 등, **그리고 자기 자신**)의 하향(`hard`→`advisory`→`off`), ② 수치 상한(`maxFRsPerSpec` 등)의 상향, ③ 등록·배제 목록·포인터·불리언 요구 스위치(`entityRegistry`·`entitySchemaSources`·`ignoreDirs`·`specDir`·`requireAccounting` 등)의 감시 표면 축소, ④ `*Exempt*`/`*Exception*` 이름 knob의 면제 개수 증가(이름 규약 자동 감지 — 새 면제 knob을 손으로 등재할 필요 없음). 정당한 재조정은 `policyRatchetExceptions`로 부채 표면화 | `"off"` |
