...
| (서식 규범) | **백틱 = entity 키 혹은 그 종속**(컬럼·필드·enum 값). **볼드+마커 = 앵커**. surface·capability 키를 백틱에 두면 FR-006 위반이고, entity 키는 반대로 백틱이 정본이다(2026-07-28 owner 결정). 즉 서식이 키의 종류를 말한다 — 데이터 모델인지 앵커인지. CLI 플래그·정책 enum 값·태그 문법(`--strict`·`advisory`·`@covers`)은 키가 아니므로 백틱을 그대로 쓴다 | knob 아님(규범) |
