...
| `frKeyAnchorPolicy` | FR 키 앵커(SPEC-023) — FR 선언 라인의 평문 bold를 그 스펙의 Ownership∪Dependencies 키와 대조(bold=키 앵커 전용, 코드 스팬은 리터럴). **entity 앵커는 `**entity** (E)` 마커 필수·(E)는 entity에만**(FR-005). `off`(판정 안 함)·`advisory`(미매치 경고)·`hard`(미매치 exit 1). **권장 종착지 = `hard`**; `off`/`advisory`는 마이그레이션 중 임시 상태 — 기존 수사적 bold·(E) 누락을 `/sdd-migrate`로 정리한 뒤 `hard`로 승격한다(update가 백로그 0에서 승격 권장) | `"off"` |
...
| `entitySchemaSources` | 위 판정의 구조 SSOT 어댑터(인프라 무관) — `[{globs:[...], patterns:["정규식"]}]`. 각 패턴의 **캡처그룹 1**이 실재 entity 식별자. 스키마 종류별 예: Drizzle `[{globs:["src/db/schema.ts"],patterns:["pgTable\\(\"([a-zA-Z0-9_]+)\""]}]` · Prisma `[{globs:["prisma/schema.prisma"],patterns:["model\\s+([A-Za-z0-9_]+)"]}]` · SQL 마이그레이션 `[{globs:["migrations/**/*.sql"],patterns:["CREATE TABLE (?:IF NOT EXISTS )?\"?([a-z0-9_]+)"]}]` · proto `[{globs:["**/*.proto"],patterns:["message\\s+([A-Za-z0-9_]+)"]}]` | `[]` |
| `entitySchemaExemptEntities` | 스키마엔 없지만 정당한 aggregate(외부 API 자원·이벤트 스트림 등) 면제 — `{"<entity>":"<사유>"}`. 빈 사유는 ownership 게이트 exit 1(entityRegistry 동형, 남용 방지) | `{}` |
