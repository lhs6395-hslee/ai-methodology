...
// CI/CD 파이프라인 셋업 인터뷰 순수 코어 (SPEC-059, Phase 1) — IO 없음.
//
...
// 직렬화**·**검증**하는 순수 함수만 제공한다. 실행 파일 쓰기는 렌더러(jenkins-renderer.mjs)의 몫.
//
...
export function validatePipelineConfig(config) {
  ...
  for (const p of c.promotions || []) {
    ...
    // (d) 인프라 승인 스텝을 켰는데 배포 대상에 v1 렌더러 템플릿이 없으면(Jenkins는 kubernetes만 지원) 명시한다.
    if (c.infraApply && c.infraApply.mode && c.deployTarget !== "kubernetes") {
      out.push({ kind: "infra-apply-no-renderer-template", at: `${p.from}→${p.to}`,
        detail: `대상 인프라 "${c.deployTarget}"의 인프라 적용 승인 스텝은 설정만 저장된다 — Jenkins 렌더러 v1은 kubernetes 템플릿만 갖고 있다` });
    }
  }
  ...
  if (c.ephemeralAgent === "unknown") {
    out.push({ kind: "ephemeral-agent-unknown-defaulted", at: "-",
      detail: "CI 에이전트가 매 실행 새로 뜨는지 모른다고 답해 안전측(자가조달 포함)으로 기본 적용했다 — Jenkins라면 node 자가조달 블록이 항상 포함된다" });
  }
