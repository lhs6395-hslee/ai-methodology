...
console.log(`Spec 완전성 게이트: spec ${files.length}개 검사 (FR 있는 spec은 SC·인수조건, Reviewed 이상은 리뷰 기록 필요).`);
if (findings.length) {
  const tag = STRICT ? "✗" : "⚠";
  console.log(`${tag} 완전성 미흡 ${findings.length}건:`);
  for (const f of findings) console.log(`  ${tag} ${f.specId}: ${f.miss}`);
  if (STRICT) { console.error(`\n✗ --strict: FR 있는 spec은 SC·인수조건, Reviewed 이상은 리뷰 기록 필요.`); process.exit(1); }
  process.exit(0);
}
console.log(`✓ 완전성 구비 — SC·인수조건·수명주기 기록 모두 충족.`);
