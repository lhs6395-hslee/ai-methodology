...
import {
  ...
  deployPreconditionFindings, deployPreconditionVerdict,
...
const findings = deployPreconditionFindings({ dirty, behind, upstream, branch }, parsed.paths);
...
console.log(`[SDD 배포 전제] \`${parsed.tool}\` — 이 배포가 재현 가능한 리비전에서 나오는가(deployPreconditionPolicy=${POLICY})`);
