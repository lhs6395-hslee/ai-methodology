...
// 분류 접미 `(security)`·`(performance)`를 허용한다 — FR 선언 정규식은 EARS 분류 `(event)`를
// 받는데 SC/NFR만 안 받아서, `**NFR-001** (security):` 형태가 **집계에서 조용히 빠졌다**
// (실측 제보: 미회계로도 안 잡히고 항목 수에서 사라짐 — 회계 게이트가 자기 사각을 못 본 것).
export const SC_DECL_RE = /^\s*[-*]\s+\*\*(SC|NFR)-(\d+)\*\*\s*(?:\([^)]*\))?\s*:/;
