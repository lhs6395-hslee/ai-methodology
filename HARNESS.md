...
| **R3 dedup+입도+완전성+일관성** | spec 생성/변경 직후 | `check-ownership`(+`entityRegistry`)·`check-spec-cohesion`·`check-spec-completeness`(SC·인수조건 + 수명주기 기록)·`check-spec-consistency` | "중복 통합 / 과대 spec 분할 / SC·인수조건·리뷰 기록 보강 / 근거 없는 키 정렬?" | 통합·분할·보강 → 재검증 |
