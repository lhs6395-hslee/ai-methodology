...
**동기화가 필요한 축은 spec↔code 하나가 아니다 — 인프라↔IaC도 같은 원칙을 받는다(R2″).** 실측 제보(2026-08-22): Terraform으로 SNS 알림 기능을 걷어내는 작업에서 `plan`을 실행했더니 의도한 리소스 삭제 외에 무관한 변경 6건(bastion 재생성·CloudFront·Lambda 등)이 섞여 나왔다 — 건드린 파일과 무관했고, git에 커밋된 IaC 코드와 실제 라이브 인프라가 이미 어긋나 있었다. `check-spec-sync`가 애플리케이션 코드에서 강제하는 것과 정확히 같은 계급의 결함이 인프라 계층에서 그대로 재현된 것이다. 이 킷은 이 축을 **두 시점**에서 잡는다: ①**주기·가오픈 점검**(선언과 실물의 차이를 사후에 측정 — `liveRealityChecks`, SPEC-032) ②**배포 직전 실시간 차단**(계획된 변경이 이 changeset 범위를 벗어나면 apply를 막는다 — `deployScopeCommand`, SPEC-035 FR-009, 레시피는 `tooling/sdd.config.presets.md` §배포 전제조건 템플릿). 즉 이 방법론을 IaC와 함께 채택하는 프로젝트는 애플리케이션 코드만이 아니라 **인프라도 동기화 대상**이라고 처음부터 읽어야 한다 — 세 축 모두 같은 형태다: 코드↔스펙(R1·R2), 코드 기대 스키마↔실제 DB(R2′), 계획된 인프라 변경↔실제 changeset 범위(R2″).
...
**라이브 대조의 두 축(SPEC-032) — 틀이 있는 것과 그 틀이 이 산출물을 본다는 것은 다르다:** R9는 저장소 밖 진실을 보는 틀인데, **새 배포 산출물을 선언할 때 대응 검사 등록을 요구하지 않으면** 그 산출물은 여전히 아무도 안 본다. 실측 제보(qa에이전트 도입): 배포 산출물 결함 8건을 **배포로 하나씩** 발견했다 — ECR 리포 없음·크로스계정 ECR API·**아치 불일치(arm64 노드에 amd64 이미지)**·**이미지에 의존 모듈 누락** 등. 전부 저장소 밖 사실이고 전부 로컬 게이트를 green으로 통과했으며, R9 검사 항목 6건에 그 중 하나도 없었다. 그래서 R9를 **등록**(선언된 배포 산출물에 담당 검사가 있는가)과 **실행**(그 검사가 라이브를 봤는가)으로 가른다. ⚠ **정책까지 갈라야 한다**: 실행은 자격증명이 필요해 흔히 off·skipped이고 훅에서 위임되는데, 한 정책에 묶여 있으면 실행을 끄는 순간 **등록 누락까지 함께 안 보인다** — 그게 제보의 구조였다. 등록은 순수 선언 대조라 오프라인에서도 판정된다. 담당 선언(`covers`) 없는 검사는 아무것도 커버하지 않는다(그걸 세면 "검사가 하나라도 있으면 통과"가 된다). 그리고 **증거 등급도 갈린다**: "단위테스트 통과"와 "배포본에서 실제 실행됨"은 같은 증거가 아니므로, 배포 산출물을 **소유한** 스펙의 배포 대상 주장은 배포 등급 증거를 요구한다 — 단 트리거는 소유와 주장 둘 다여야 한다(마커만으로 걸면 배포를 다루기만 하는 스펙까지 잡히고, 그런 스펙의 정답 증거는 단위테스트다).

**검증 실행 회계(SPEC-041) — 존재는 실행이 아니다:** `[검증: <경로>]`가 실재하는 자산을 지목하는 것(SPEC-031)까지 왔는데, 그다음 질문이 없었다 — **그 자산이 돌았는가.** 실측 제보에서 검증이 세 번 조용히 사라졌다: 에이전트가 재현 데이터가 없다며 중도 포기했고(산출물 0), 러너가 대상 0건으로 `exit 0` 종료해 성공과 무행동이 동형이 됐고, 배포 훅 Job이 참조 이미지 부재로 한 번도 뜨지 않았는데 비차단이라 파이프라인은 초록이었다. spec↔code 정합만 보는 층에서 이 셋은 전부 통과다. 그래서 **실행 원장**(`verificationRunLedger`, JSONL)을 판정 입력으로 두고 선언된 증거와 대조한다 — 러너·CI 스테이지·에이전트가 자기 결과를 `--record`로 남기고 `outcome`은 SPEC-040의 다섯 종류를 그대로 쓴다. **차단하는 것은 침묵과 깨진 기록뿐이다**: 기록이 아예 없으면(=안 돈 것과 구분되지 않으면) 위반이고, 사유 없이 "못 했다"만 적은 줄도 기록이 아니다. 반대로 **사유가 붙은 미실행은 어떤 강도에서도 막지 않고** 매 실행 부채로만 표면화한다 — 포기 자체는 정당하게 일어나고, 그것까지 막으면 사람이 사유를 지어내 원장이 거짓말을 담기 시작한다. 원장은 세션·CI 로컬 상태라 커밋하지 않는다(남의 실행 기록이 내 체크아웃에서 "돌았다"로 읽히는 것이 정확히 이 층이 막으려는 거짓이다).
