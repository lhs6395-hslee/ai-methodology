...
**게이트 판정 타입(SPEC-040) — "초록"의 자격은 하나뿐이다:** 게이트가 `exit 0`으로 끝났다는 것과 게이트가 **판정했다**는 것은 다른 사실이다. 이 규범은 오래 산문으로만 존재했고 그래서 하네스 자신이 어겼다 — 게이트가 `engineRealityPolicy·eventAttributionPolicy 모두 off (판정 안 함)`이라고 정직하게 적은 줄을, 집계기가 `⚠`·`✗`가 없다는 이유로 `✓ clean`으로 분류했다(실측: R7·R9). **고칠 것은 어휘가 아니라 반환 타입이다** — 문자열 스캔은 새 게이트가 새 표현을 쓰는 순간 조용히 틀리고 그 실패는 언제나 초록으로 나타난다. 이제 게이트는 자기 판정의 **종류**를 선언하고 집계기는 그것을 읽기만 한다: `JUDGED`(봤고 결과를 안다 — **유일하게 초록의 자격**) · `OFF`(사람이 껐다) · `INERT`(켰는데 볼 것이 없다) · `SKIPPED`(사유 있는 생략) · `UNTYPED`(선언 안 함 = 배선 누락). 종류는 다섯뿐이고 늘리지 않는다 — 늘리면 "이건 어디에 넣지"가 생기고 그 자리가 예외가 된다. `OFF`와 `INERT`를 가르는 이유는 **해소가 다르기 때문**이다(켜면 됨 vs 입력을 만들어야 함); 묶으면 "켰으니 됐다"가 성립해 *hard 선언 + 무판정 = 거짓 안전*이 되살아난다. 스윕은 규칙을 **세 상태**(판정함 · 안 봄 · 위반)로 렌더하고 요약은 **언제나** 분모를 낸다 — `게이트 N종 = 판정 M · 안 봄 K(off·inert·생략) · 미판정 J`. `--strict`가 막는 것은 위반과 미판정뿐이다: `OFF`·`INERT`는 막지 않되 초록에 합산하지도 않는다(채택 중인 프로젝트를 벽으로 막으면 사람이 정책을 끈다 — 우회를 유발하는 강제는 강제가 아니다). 같은 결함의 다른 얼굴이 **기본값의 언어 편향**이다: 구현 중복 게이트의 기본 파일 패턴이 JS/TS라서 Python 저장소는 아무 파일도 안 보고 "중복 0건"을 보고했고 그 0은 진짜 0과 구분되지 않았다 — 이제 언어를 선언하지 않은 채 일부만 본 상태는 `INERT`이고 게이트가 **무엇을 안 봤는지**(확장자·건수) 지목한다.
...
| 35 | 이 테스트가 **정말 그 FR을 검증하는가**(결속이 맞아도 내용은 다를 수 있다) | 양쪽이 **서로를 지목했는지**만 기계 판정(SPEC-039): 태그의 파일이 FR의 `[검증]` 목록에 있는가. `[검증]`이 없는 FR은 위반이 아니라 미표기 부채로 따로 센다. 실재만 보는 단방향 검사는 **번호가 겹치면 통과**한다 — 실재는 동일성이 아니다 | 그 테스트가 FR 문장의 계약을 실제로 재현하는지 — 스펙 리뷰. 기계는 상호 지목만 센다. **`[검증]` 표기를 붙이는 순간 그 FR이 이 검사의 보호를 받는다**는 점이 표기의 유인이다 |
