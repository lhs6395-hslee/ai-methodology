...
4. **따라서 실제 순서는 3단이다: ① FR 앵커 밀도 확보 → ② aggregate 단위 분할 → ③ capability 이관.** ①을 건너뛰고 ②를 하면 실행자가 124개 FR의 귀속을 *추정*하게 되고, 그 추정은 스펙 경계를 대신 결정하는 것이다. ①은 판단이 아니라 저술이라(이미 문장이 말하는 대상을 표기) 위임 가능하지만 ②는 리뷰 경계다.

이관이 막힌 것처럼 보일 때 진짜 블로커는 이관이 아니라 **그 위의 두 단계**다.
